 ...
 * <li>서브 업무명 : BSC002</li>
 * <li>설  명 : <pre>[IF]처리상태전송</pre></li>
 * <li>작성일 : 2015-07-30 15:18:31</li>
 * <li>작성자 : 이영진 (newnofixing)</li>
 * </ul>
 */
public class IFE006 extends AbsBatchComponent {
    ...
    public void execute(final IBatchContext context) {
        ...
        try {
            ...
            OutputStream fout = dbToFile.HeaderParseData(paramMap);
            
            StringBuffer bf = new StringBuffer();
            bf.append(paramMap.get("TASK_ID"));
            bf.append(".SKCC.");
            ...
            /* DB TYPE 은 NUMBER 지만 파일로 만들때는  VARCHAR처럼 뒤로 스페이스 들어가게 만들어야 함. */

            body.addColumnInfo("CNSL_MGMT_NO",			10, SAMFileTool.TYPE_STRING);
            body.addColumnInfo("EQP_MDL_CD",			10, SAMFileTool.TYPE_STRING);
            body.addColumnInfo("EQP_SER_NO",			15, SAMFileTool.TYPE_STRING);
            body.addColumnInfo("PROGR_ST",				 2, SAMFileTool.TYPE_STRING);
            body.addColumnInfo("PAY_CL",				 2, SAMFileTool.TYPE_STRING);
            body.addColumnInfo("PAY_DT",				 8, SAMFileTool.TYPE_STRING);
            body.addColumnInfo("CNSL_AMT",				 7, SAMFileTool.TYPE_STRING);
//            body.addColumnInfo("CNSL_AMT",				 7, SAMFileTool.TYPE_BIGDECIMAL);
            body.addColumnInfo("EQP_ST",				 2, SAMFileTool.TYPE_STRING);
            body.addColumnInfo("SKN_SMPL_JDG_DAMT",		 7, SAMFileTool.TYPE_STRING);
//            body.addColumnInfo("SKN_SMPL_JDG_DAMT",		 7, SAMFileTool.TYPE_BIGDECIMAL);
            body.addColumnInfo("EQP_TLY_DT",             8, SAMFileTool.TYPE_STRING);
            body.addColumnInfo("EQP_TLY_CONT",          60, SAMFileTool.TYPE_STRING);
            body.initialize();
            
            dbSelect("SProcStTrmsLst", queryParam, makeRecordHandler(body), context);  //DB 조회
            
            preTaskSTCD = "S";
            IOnlineContext    onlineCtx  = makeOnlineContext(context);
            IDataSet reqDS = new DataSet();
            reqDS.putField("TASK_ID", "DBE006");
            reqDS.putField("TASK_DT", DateUtils.getCurrentDate());
            if (processCnt == 0) {
            ...
            dbToFile.tailParseData(paramMap, fout);
            
            dbUpdate("UProcStTrms", queryParam, context);	// 처리상태코드 Update
