...
//	        log.debug(">>>>>>>>>>>>>>>   XCL_GUBUN : "+context.getInParameter("XCL_GUBUN"));
	        ...
	        param.put("XCL_GUBUN", context.getInParameter("XCL_GUBUN"));
	    	...
	    	try {
	    	    ...
	    	    if("RENT".equals(context.getInParameter("XCL_GUBUN"))){
	    	        dbSelect("SXclRent", param, makeRecordHandler(context), readSession ); //랜트료 정산 
	    	    }else if("HD".equals(context.getInParameter("XCL_GUBUN"))){
	    	        dbSelect("SXclDlv", param, makeRecordHandler(context), readSession); //운송비 정산 
	    	    }else if("JD".equals(context.getInParameter("XCL_GUBUN"))){
	    	        dbSelect("SXclCmp", param, makeRecordHandler(context), readSession); //손해배상 정산 
	    	    }else if("JN".equals(context.getInParameter("XCL_GUBUN"))){
	    	        dbSelect("SXclJN", param, makeRecordHandler(context), readSession); //손해배상 미반납 정산 
	    	    }else if("RC".equals(context.getInParameter("XCL_GUBUN"))){
	    	        param.put("RE_CMC", "Y");
	    	        dbSelect("SXclCmp", param, makeRecordHandler(context), readSession); //재상품화수수료 정산 
	    	    }else if("RR".equals(context.getInParameter("XCL_GUBUN"))){
	    	        dbSelect("SXclCmpt", param, makeRecordHandler(context), readSession); //구성품 정산 
	    	    }
...
//	    	    dbSelect("SXclBond");
			...
			log.debug(">>>>>>>>>>>>>>>   processCnt : "+processCnt);
			if(processCnt > 0){
				// 트랜잭션 시작
				txBegin();  
				dbStartSession();
				dbBeginBatch();
				IRecord map= dbSelectSingle("SXclBond", param, context);
				dbInsert("IXclBond", map);
	    ...
	    public AbsRecordHandler makeRecordHandler(IBatchContext context) {
	    	AbsRecordHandler rh = new AbsRecordHandler(context) {
				...
				public void handleRecord(IRecord row) {
					...
					map.put("SALE_DEALCO_CD", "0000000000");
					map.put("OP_CL_CD", "PR");
					map.put("XCL_GUBUN", context.getInParameter("XCL_GUBUN"));
