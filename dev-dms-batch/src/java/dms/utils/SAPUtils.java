...
import dms.constants.SlipConstants;
...
public class SAPUtils {
	 ...
	 * @return
	 */
	public static Map convertRecord2Map(IRecord ir)
	{
		Map responseMap = new HashMap();
		
		if(ir==null || ir.isEmpty()) return responseMap;
		Set s = ir.keySet();
	 ...
	 * @return
	 */
	public static IDataSet convertMap2DataSet(Map ir)
	{
		IDataSet responseData = new DataSet();
	 ...
	 * @return
	 */
	public static IRecordSet convertMap2IRecordSet(String rsName, Map map)
	{
		IRecordSet responseData = new RecordSet(rsName);
		
		if(map==null || map.isEmpty()) return responseData;
		Set s = map.keySet();
		Iterator i = s.iterator();
		...
		{
			key = (String) i.next();
			responseData.addHeader(new RecordHeader(key));
		}
		
		IRecord recordContents = responseData.newRecord();
		
		i = responseData.getHeaders();
		while(i.hasNext())
		{
			RecordHeader header = (RecordHeader) i.next();
			recordContents.put(header.getName(), map.get(header.getName()));
		}
		
		responseData.addRecord(recordContents);
	 ...
	 * @return IRecordSet
	 */
	public static IRecordSet putColumnAll(IRecordSet irs, String key, String value)
	{
	    IRecordSet returnRs = irs;
	    if(irs!=null && irs.getRecordCount()>0)
	    {
	        IRecord ir ;
	        if(!hasHeaderName(irs, "key"))
	        {
	            returnRs.addHeader(new RecordHeader(key));
	        }
	        for(int i=0;i<returnRs.getRecordCount();i++)
	        {
	            ir = returnRs.getRecord(i);
	            ir.put(key, value);
	            returnRs.setRecord(i, ir);
	        }
	    }
	    return returnRs;
	}
	...
	/**
     * ds가 가지고 있는 모든 레코드셋에 다가 해당 키값으로 다 덮어 쓴다. 
     * putAll
     *  
     * @param irs
     * @param key
     ...
     * @return IRecordSet
     */
    public static IDataSet putColumnAll(IDataSet ids, String key, String value)
    {
        IDataSet returnDs = ids;
        if(ids!=null && ids.getRecordSetCount()>0)
        {
            Iterator<String> it = ids.getRecordSetIds();
            String rsName;
            
            while(it.hasNext())
            {
                rsName = it.next();
                IRecordSet irs = putColumnAll(ids.getRecordSet(rsName),key,value); 
                ids.putRecordSet(rsName, irs);
            }
        }
        return returnDs;
    }
	...
	public static String nvl(Map map , String key, String defaultValue)
	{
		if(map!=null && map.containsKey(key))
		{
			...
			{
				returnValue = (BigDecimal) tmpObj+"";
			}
			else
			{
				returnValue = (String) tmpObj+"";
			}
			return returnValue;
		}
		else
	...
	/**
	 * 단하나의 값으로 리스트을 만들어 리턴한다.
	 * @param key
	 * @param value
	 * @return
	 */
	public static List newList(String value)
	{
     ...
     * @param map
     * @param prefix
     * @param trimFlag substring next prefix 
     * @return Map
     */
    public static Map extractMapByPrefix(Map map, String prefix, boolean trimFlag)
    {
        Map returnMap = new HashMap();
        ...
        while(it.hasNext())
        {
            ...
            if(key.startsWith(prefix))
            {
                if(trimFlag)
                {
                    returnMap.put(key.substring(prefix.length()), map.get(key));
                }
                else
                {
    ...
    public static IDataSet prepareXMLInputParam(IBatchContext context, String parameterName)
    {
    	...
    	if(StringUtils.isNotEmpty(reqDataSetStr))
    	{
    		requestData    = DataSetXmlTransformer.xmlToDataSet(new java.io.StringReader(reqDataSetStr));
     ...
     * @return
     */
    public static boolean isNotEmpty(IRecordSet rs)
    {
    	boolean returnFlag = false;
     ...
     * @return
     */
    public static String getMM_YYYYMMDD(String yyyyMMdd)
    {
    	String returnStr = yyyyMMdd;
    	
    	if(StringUtils.isNotEmpty(yyyyMMdd) && yyyyMMdd.length() == 8)
    	{
    		returnStr =  yyyyMMdd.substring(4, 6);
    	}
    	return returnStr;
    }
     ...
     * @return
     */
    public static String getDD_YYYYMMDD(String yyyyMMdd)
    {
        String returnStr = yyyyMMdd;
        
        if(StringUtils.isNotEmpty(yyyyMMdd) && yyyyMMdd.length() == 8)
        {
            returnStr =  yyyyMMdd.substring(6, 8);
        }
        return returnStr;
    }
     ...
     * @return
     */
    public static String getYYMMDD_YYYYMMDD(String yyyyMMdd)
    {
        String returnStr = yyyyMMdd;
        
        if(StringUtils.isNotEmpty(yyyyMMdd) && yyyyMMdd.length() == 8)
        {
            returnStr =  yyyyMMdd.substring(2, 8);
        }
        return returnStr;
    }
     ...
     * 존재하는지
     */
    public static boolean in(Object arr, Object searchObj )
    {
    	return in(new Object[]{arr}, searchObj);
    }
    ...
    public static boolean in(Object[] arr, Object searchObj )
    {
    	...
    	if(searchObj == null  || ArrayUtils.isEmpty(arr)) return returnFlag;
    	
    	for(Object one :arr)
    	{
    		if(one.equals(searchObj))
    		{
    			returnFlag = true;
    			break;
    		}
     ...
     * @return
     */
    public static IRecordSet nvl(IRecordSet rs)
    {
    	if(isNotEmpty(rs)) return rs;
    	else return new RecordSet("");
    }
