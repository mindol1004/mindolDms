    ...
    public AutoCommitRecordHandler makeRecordHandler(IBatchContext context) {
        ...
        AutoCommitRecordHandler rh = new AutoCommitRecordHandler(this,context,  dbNewSession(context, false)) {
            ...
            public void handleRecord(IRecord row) {
                ...
                paramMap.put("CNSL_DEALCO_CD"         , StringUtils.nvlObject(row.get("DEALCO_CD"),""));          // 상담조직코드          
                ...
                paramMap.put("UKEYREG_DTM"            , StringUtils.nvlObject(row.get("UKEY_REG_DTM"),""));          // UKEY등록일시                 
                paramMap.put("UKEYREG_USER_ID"        , StringUtils.nvlObject(row.get("UKEY_REG_USER_ID"),""));      // UKEY등록사용자ID      
                paramMap.put("FST_RGSTR"              , StringUtils.nvlObject(row.get("UKEY_FST_REG_USER_ID"),""));  // UKEY최초등록사용자ID  
                paramMap.put("UKEYCHG_DTM"            , StringUtils.nvlObject(row.get("UKEY_CHG_DTM"),""));          // UKEY변경일시          
                paramMap.put("UKEYCHG_USER_ID"        , StringUtils.nvlObject(row.get("UKEY_CHG_USER_ID"),""));      // UKEY변경사용자ID  
                ...
                paramMap.put("UKEY_XCL_DEALCO_ORG_ID_2" , StringUtils.nvlObject(row.get("UKEY_XCL_ORG_ID_2"),""));     // UKEY정산조직ID2      
                paramMap.put("PRE_DC_CL"              , StringUtils.nvlObject(row.get("DCINAD_CL"),""));         // 선할인구분           
