...
public class XCR009 extends AbsBatchComponent {
    ...
    public void execute(final IBatchContext context) {
        ...
        {
            IRecordSet rs = dbSelectMulti("SInvLst", paramMap);
            boolean flagContinue = false;
            for(int i=0; i<rs.getRecordCount(); i++) //모수
            {
                for(Map one:doNotEtcDeleteList) //비교 기존 집계 자료에서 전표진행중인것들
                {
                    if(this.equalsMap4Key(one, rs.getRecordMap(i), new String[]{"ASSET_NO","CNTRT_NO","ETC_XCL_TS","XCL_DEPT_CD","XCL_DT","XCL_DEALCO_CD","SGI_INSU_NO"}))
                    {
                        flagContinue = true;
                        break;
                    }
        ...
        {
            this.upsertHandler(context, paramMap, "SInvLst", "IEtcXcl");
        }
        
        dbCommitSession();
        
        dbDelete("DXclBondByYM", paramMap);
        
        this.upsertHandler(context, paramMap, "SInvSumLst", "IBond");
     ...
     */
     ...
     private IRecord processWithinUpsertRecordHandler(IBatchContext context, IRecord row)
     {
		 ...
		 IOnlineContext    onlineCtx  = makeOnlineContext(context);
		 IDataSet reqDS = new DataSet();
		 reqDS.putField("DEALCO_BLICENS_NO", row.get("XCL_BIZ_REG_NO"));
		 IDataSet resDS = callOnlineBizComponent("nr.NRSXMBase", "fInqVendorAtERP", reqDS, onlineCtx);
		 row.put("DEALCO_FISCL_ST_CD",this.getSkipCustomer(row.get("BIZ_REG_NO"), resDS.getField("ERP_MSG_CD")));
		 //row.put("DEALCO_FISCL_ST_DESC",resDS.getField("ERP_MSG_DESC"));
