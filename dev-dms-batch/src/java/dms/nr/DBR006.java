    ...
    public AbsRecordHandler makeRecordHandler(IBatchContext context) {
    	rh = new AutoCommitRecordHandler(this, context,  dbNewSession(context, false)) {
			...
			public void handleRecord(IRecord row) {
				...
				// 회수테이블 일련번호,모델코드로 조회해서 상태값이 03 OR 04 이면  건너띄고 진행함.	
				IRecord clctRs = dbSelectSingle("SEqpClctObj",reqDs.getFieldMap());
				if( !StringUtils.isEmpty(clctRs.get("IN_OBJ_DTL_CD")) ){
					
					if( !("03".equals(clctRs.get("IN_OBJ_DTL_CD")) ||  "04".equals(clctRs.get("IN_OBJ_DTL_CD"))) ){
