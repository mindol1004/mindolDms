 ...
 * <li>서브 업무명 : BIF102</li>
 * <li>설  명 : <pre>[IF]단말 구성품 등록(임대R)</pre></li>
 * <li>작성일 : 2015-08-11 15:39:38</li>
 * <li>작성자 : 안한모 (ahmlee)</li>
 * </ul>
 */
public class IFP002 extends AbsBatchComponent {
    ...
    public void execute(final IBatchContext context) {
        ...
        try {
            ...
            body.addColumnInfo("EQP_EFF_STA_DT",            8, SAMFileTool.TYPE_STRING);
            body.addColumnInfo("EQP_EFF_END_DT",            8, SAMFileTool.TYPE_STRING);
            body.addColumnInfo("EQP_CMPT_CD",               2, SAMFileTool.TYPE_STRING);
            body.addColumnInfo("EQP_CMPT_NM",              50, SAMFileTool.TYPE_STRING);
            body.addColumnInfo("EQP_CMPT_AMT",             11, SAMFileTool.TYPE_STRING);
            body.initialize();

            dbSelect("SPrEqpCmptRegLst", queryParam, makeRecordHandler(body), context);  //DB 조회
            
            //20150911 start
            preTaskSTCD = "S";
            IOnlineContext    onlineCtx  = makeOnlineContext(context);
            IDataSet reqDS = new DataSet();
            reqDS.putField("TASK_ID", "DBP002");
            reqDS.putField("TASK_DT", DateUtils.getCurrentDate());
            if (processCnt == 0) {
            ...
            dbUpdate("UPrEqpCmptReg", queryParam, context);	// 처리상태코드 Update
