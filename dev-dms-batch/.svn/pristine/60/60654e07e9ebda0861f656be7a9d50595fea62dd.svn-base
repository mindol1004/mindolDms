...
public class XCR016 extends AbsBatchComponent {
    ...
    public void execute(final IBatchContext context) {
        ...
        paramMap.put("TS", context.getInParameter("TS"));//#정산차수
        
        //기존 기타정산테이블에 해당월꺼 있는거 삭제하고
        dbDelete("DEtcXclCheckLst", paramMap, context);
        if( "IP".equals(context.getInParameter("XCL_ITM_CL")) ){
            dbDelete("DDebtXclCheckLst", paramMap, context);
        }
        else
        {
            dbDelete("DBondXclCheckLst", paramMap, context);
        }
...
//                	}
                    ...
                    SAPUtils.debug("makeXclRecordHandler row:"+SAPUtils.convertRecord2DataSet(row));
