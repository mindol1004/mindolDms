...
//	    	    	제각상세
	    	    	dbSelect("SXclAssetOutDtl", param, makeRecordHandler(context), readSession);
	    	    	
	    	    	log.debug(">>>>>>>>>>>>>>>  제각 그룹 호출 ");
	    	    	//제각마스터
	    	    	dbSelect("SXclAssetOutGrp", param, makeRecordSuperHandler(context), readSession);
							...
							if(rs!= null){
								...
								for (int i = 0; i < rs.getRecordCount(); i++) {
									...
									//전표번호가 있으나  미발행, 전표삭제 상태일경우 삭제
										...
										dCnt = dbDelete("DAssetDispoGrp", rd);
										dCnt = dbDelete("DAssetDispoDtl", rd);
									}
