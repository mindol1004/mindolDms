...
//    	IRecordSet rs = requestData.getRecordSet("RS_SLIP_DELETE");
    	...
    	for(int i=0; i<rs.getRecordCount(); i++)
    	{
    	    ...
    	    if("EP_Y".equals(requestData.getField("SLIP_TYPE")))//판매일때 판매
    	    {
    	        dbUpdate("UEPEqpSale", paramMap);
            }
    		dbUpdate("UEPInveMovXcl", paramMap);
    		dbUpdate("UEPDcinad", paramMap);
    		dbUpdate("UEPIncenxcl", paramMap);
	...
	*/
	public IDataSet fRemoveSlip(IDataSet requestData, IBatchContext batchCtx) {
		...
		List<String> slipNoFields = new ArrayList();
		slipNoFields.add("SLIP_NO");
		
		if("NR_OA".equals(requestData.getField("SLIP_TYPE")))
		{
			slipNoFields.add("ACQR_CNCL_SLIP_NO");
		}
		else if("NR_OC".equals(requestData.getField("SLIP_TYPE")))
		{
			slipNoFields.add("DEPR_CNCL_SLIP_NO");
		}
		else if("EP_D".equals(requestData.getField("SLIP_TYPE")))//원가전표삭제
        {
		    slipNoFields.removeAll(slipNoFields);
		    slipNoFields.add("INVE_PCOST_SLIP_NO");//원가만 지운다.
        }
