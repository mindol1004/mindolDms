...
public class PEPEqpPreDcXclMgmt extends fwk.base.ProcessUnit {
	...
	public IDataSet pSavePreDcXclRpetCalc(IDataSet requestData, IOnlineContext onlineCtx) {
        ...
        try {
            ...
            HashMap<String,String> params = new HashMap<String,String>(); // 배치 파라미터 값 정보.
	...
	public IDataSet pSEqpDcinadXclLst(IDataSet requestData, IOnlineContext onlineCtx) {
        ...
        try {
            ...
            if(null != resDS && "B".equals(resDS.getField("BAT_TASK_PROC_ST_CD"))){
                throw new BizRuntimeException("DMS00168", new String[] {"재집계"}); // {0} 진행중 입니다.
            }
	...
	public IDataSet pSEqpDcinadXclDtlExcelLst(IDataSet requestData, IOnlineContext onlineCtx) {
        ...
        try {
            // 1. FM 호출        
            responseData = callSharedBizComponentByDirect("ep.EPSSXBase", "fSEqpDcinadXclDtlExcelLst", requestData, onlineCtx);
        } catch ( BizRuntimeException e ) {
            throw e;
        } catch ( Exception e ) {
            throw new BizRuntimeException("DMS00009", e); // 시스템 오류가 발생하였습니다.
...
//            	throw new BizRuntimeException("DMS00166");
            	...
            	if(result == -1) throw new BizRuntimeException("XYZE0010"); // 시스템 오류가 발생하였습니다.             
...
//            responseData = callSharedBizComponentByDirect("nr.NRSXMBase", "fSlipInfoSynchronization", requestData, onlineCtx);
	 ...
	 * @return 처리결과 DataSet 객체
	 */
	public IDataSet pInqEqpPreDcXclTaxBillSearch(IDataSet requestData, IOnlineContext onlineCtx) {
	    IDataSet responseData = new DataSet();
	    
        try {
               // 1. FM 호출
               responseData = callSharedBizComponentByDirect("ep.EPSSXBase", "fInqEqpPreDcXclTaxBillSearch", requestData, onlineCtx);
           } catch ( BizRuntimeException e ) {
               throw e;
           } catch ( Exception e ) {
               throw new BizRuntimeException("DMS00009", e); // 시스템 오류가 발생하였습니다.
	 ...
	 * </pre>
	 */
	public IDataSet pInqEqpPreDcXclExcelErrLst(IDataSet requestData, IOnlineContext onlineCtx) {
        IDataSet responseData = new DataSet();
    
        try {
            // 1. FM 호출
            responseData = callSharedBizComponentByDirect("ep.EPSSXBase", "fInqEqpPreDcXclExcelErrLst", requestData, onlineCtx);
	 ...
	 * @return 처리결과 DataSet 객체
	 */
	public IDataSet pSaveEqpPreDcXclExcel(IDataSet requestData, IOnlineContext onlineCtx) {
	    IDataSet responseData = new DataSet();
        CommonArea ca = getCommonArea(onlineCtx);
        ...
        try {
            ...
            responseData = callSharedBizComponentByDirect("ep.EPSSXBase", "fSaveEqpPreDcXclExcel", requestData, onlineCtx);
