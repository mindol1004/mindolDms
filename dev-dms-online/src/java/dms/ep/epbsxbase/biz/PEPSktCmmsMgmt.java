	...
	public IDataSet pSaveSktCmmsRpetCalc(IDataSet requestData, IOnlineContext onlineCtx) {
        ...
        try {                                                                                 
            ...
            */
            ...
            rowInstDs.putField("CMMS_XCL_DTL_SEQ", insertRow);      // 요금선납미착재고 대상 이후_수수료 정산 상세 순번
            // 2.6 클럽T 현장감정 SKT수수료집계
            insertRow += Integer.parseInt(callSharedBizComponentByDirect("ep.EPSSXBase", "fIClubTSktCmms", rowInstDs, onlineCtx).getField("insertRow"));
	...
	public IDataSet pSSktCmmsDtlLst(IDataSet requestData, IOnlineContext onlineCtx) {
        ...
        try {
            ...
            */
            ...
            }else if("04".equals(StringUtils.nvlObject(requestData.getField("DEDC_TYP_CD"),"")) && "10".equals(StringUtils.nvlObject(requestData.getField("INVE_TYP_CD"),""))){
                // FM 호출 : [FM]클럽TSKT수수료목록조회 (클럽T && 확정재고)
                responseData = callSharedBizComponentByDirect("ep.EPSSXBase", "fSClubTSktCmmsLst", requestData, onlineCtx);
            }
