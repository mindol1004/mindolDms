...
public class PEPInveXclMgmt extends fwk.base.ProcessUnit {
	 ...
	 * @return 처리결과 DataSet 객체
	 */
	public IDataSet pDelInveXcl(IDataSet requestData, IOnlineContext onlineCtx) {
        IDataSet responseData = new DataSet();
    
        try {
            // 1. FM 호출
            responseData = callSharedBizComponentByDirect("ep.EPSSXBase", "fDelInveXcl", requestData, onlineCtx);
            // 2. 결과값 리턴
            responseData.setOkResultMessage("DMS00000", null); //정상 처리되었습니다.
        } catch ( BizRuntimeException e ) {
            throw e;
