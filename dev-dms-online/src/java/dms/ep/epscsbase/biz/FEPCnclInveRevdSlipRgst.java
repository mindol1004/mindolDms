...
public class FEPCnclInveRevdSlipRgst extends fwk.base.FunctionUnit {
	...
	public IDataSet fRegCnclInveRevdSlipRgst(IDataSet requestData, IOnlineContext onlineCtx) {
        ...
        try {
            ...
            String feeDedcCnclFlag = ""; //요금공제취소여부
            ...
            for(int i = 0; i < rsList.getRecordCount(); i++){
                ...
                if(resData != null && resData.getRecordSetCount() > 0 && resData.getRecordSet("RS_CNCL_INVE_EQP_XCL_INFO") != null && resData.getRecordSet("RS_CNCL_INVE_EQP_XCL_INFO").getRecordCount() > 0){
                    ...
                    for(int j = 0; j < rsInveList.getRecordCount(); j++){
                        tmpData.putFieldMap(rsInveList.getRecordMap(j));
                        if("20".equals(tmpData.getField("INVE_TYP_CD").toString()) && "N".equals(tmpData.getField("CNCL_SLIP_YN").toString())){
                            //미착재고
                        ...
                        }else if("10".equals(tmpData.getField("INVE_TYP_CD").toString()) && "N".equals(tmpData.getField("CNCL_SLIP_YN").toString())){
                            //확정재고
                            if(rsArrivList == null) {
                ...
                resData = new DataSet();
                if(reqData.getField("PRCH_MGMT_NO") != null && !"".equals(reqData.getField("PRCH_MGMT_NO").toString())){
                    resData = dEPCnclInveRevdSlipRgst.dSInqCnclInveFeeDedcXclInfo(reqData, onlineCtx);
                }else if(reqData.getField("CNSL_MGMT_NO") != null && !"".equals(reqData.getField("CNSL_MGMT_NO").toString())){
                    resData = dEPCnclInveRevdSlipRgst.dSInqCnclInveFeeDedcXclInfoCnsl(reqData, onlineCtx);
                }
                if(resData != null && resData.getRecordSetCount() > 0 && resData.getRecordSet("RS_CNCL_INVE_FEE_XCL_INFO") != null && resData.getRecordSet("RS_CNCL_INVE_FEE_XCL_INFO").getRecordCount() > 0){
                    rsInveList = resData.getRecordSet("RS_CNCL_INVE_FEE_XCL_INFO");
                    feeDedcCnclFlag = "N";
                    for(int j = 0; j < rsInveList.getRecordCount(); j++){
                        tmpData.putFieldMap(rsInveList.getRecordMap(j));
                        if("Y".equals(tmpData.getField("CNCL_SLIP_YN").toString())){//
                    ...
                    for(int j = 0; j < rsInveList.getRecordCount(); j++){
                        tmpData.putFieldMap(rsInveList.getRecordMap(j));
                        if("N".equals(tmpData.getField("CNCL_SLIP_YN").toString()) && "N".equals(feeDedcCnclFlag)){
                            if(rsFeeDedcList == null) {
                                rsFeeDedcList = new RecordSet("RS_CNCL_INVE_EQP_XCL_INFO",rsInveList.getRecordMap(j));
                            }else{
                                rsFeeDedcList.addRecord(rsInveList.getRecord(j));
                            }
                        }
                    }
                }
            ...
            if(rsArrivList != null && rsArrivList.getRecordCount() > 0){
                for(int i = 0; i < rsArrivList.getRecordCount(); i++){
                    ...
                    reqData = new DataSet();
                    reqData.putField("INVE_XCL_NO",strInveXclNo);
                    reqData.putField("SKN_CL",rsArrivList.get(i, "SKN_CL").toString());
                    reqData.putField("INVE_CL_CD",rsArrivList.get(i, "INVE_CL_CD").toString());
                    reqData.putField("DEDC_TYP_CD",rsArrivList.get(i, "DEDC_TYP_CD").toString());
            ...
            if(rsNotArrivList != null && rsNotArrivList.getRecordCount() > 0){
                for(int i = 0; i < rsNotArrivList.getRecordCount(); i++){
                    ...
                    reqData = new DataSet();
                    reqData.putField("INVE_XCL_NO",strInveXclNo);
                    reqData.putField("SKN_CL",rsNotArrivList.get(i, "SKN_CL").toString());
                    reqData.putField("INVE_CL_CD",rsNotArrivList.get(i, "INVE_CL_CD").toString());
                    reqData.putField("DEDC_TYP_CD",rsNotArrivList.get(i, "DEDC_TYP_CD").toString());
                    resData = dEPCnclInveRevdSlipRgst.dSInqCnclInveXclSumList(reqData, onlineCtx);
                    rsInveXclList = resData.getRecordSet("INVE_XCL_SUM_LIST");
                    //정산등록
                    if(rsInveXclList != null && rsInveXclList.getRecordCount() > 0 && Long.parseLong(rsInveXclList.get(0, "INVE_XCL_CNT").toString()) > 0){
            ...
            // 3-1. 요금공제자료생성 : 요금공제정산건은 계좌송금으로 반제전표처리한다.
            if(rsFeeDedcList != null && rsFeeDedcList.getRecordCount() > 0){
                for(int i = 0; i < rsFeeDedcList.getRecordCount(); i++){
                    //일련번호채번
                    resData = dEPCnclInveRevdSlipRgst.dSInqCnclInveAccoRmtXclSeq(tmpData, onlineCtx);
                    strAccoXclNo = resData.getField("ACCO_RMT_XCL_NO").toString();
                    ...
                    reqData.putField("ACCO_RMT_XCL_NO",strAccoXclNo);
                    reqData.putField("ACCO_RMT_XCL_DTL_SEQ",i+1);
                    reqData.putField("CNSL_MGMT_NO",rsFeeDedcList.get(i, "CNSL_MGMT_NO").toString());
                    reqData.putField("PRCH_MGMT_NO",rsFeeDedcList.get(i, "PRCH_MGMT_NO").toString());
                    reqData.putField("INVE_TYP_CD",rsFeeDedcList.get(i, "INVE_TYP_CD").toString());
                    reqData.putField("EQP_MDL_CD",rsFeeDedcList.get(i, "EQP_MDL_CD").toString());
                    reqData.putField("EQP_SER_NO",rsFeeDedcList.get(i, "EQP_SER_NO").toString());
                    reqData.putField("XCL_AMT",rsFeeDedcList.get(i, "XCL_AMT"));
                    if(rsFeeDedcList.get(i,"XCL_AMT") != null && !"".equals(rsFeeDedcList.get(i,"XCL_AMT").toString()) && !"0".equals(rsFeeDedcList.get(i,"XCL_AMT").toString())){
                        reqData.putField("XCL_AMT", Long.parseLong(rsFeeDedcList.get(i,"XCL_AMT").toString())*-1);
                    }
                    reqData.putField("ACCO_RMT_SLIP_DT",rsFeeDedcList.get(i, "FEE_PPAY_SLIP_DT"));
                    reqData.putField("ACCO_RMT_SLIP_NO",rsFeeDedcList.get(i, "FEE_PPAY_SLIP_NO"));
                    reqData.putField("FS_REG_USER_ID", ca.getUserNo());
                    reqData.putField("LS_CHG_USER_ID", ca.getUserNo());
                    //정산상세등록
                    dEPCnclInveRevdSlipRgst.dICnclInveAccoRmtXclDtlRgst(reqData, onlineCtx);
                    ...
                    if(rsAccoXclList != null && rsAccoXclList.getRecordCount() > 0 && Long.parseLong(rsAccoXclList.get(0, "ACCO_RMT_XCL_CNT").toString()) > 0){
                        ...
                        reqData.putField("XCL_STRD_STA_DT", sCurrDt);
                        reqData.putField("XCL_STRD_END_DT", sCurrDt);
                        reqData.putField("INVE_TYP_CD", "40");
                        reqData.putField("CNCL_SLIP_YN", "Y");
                        reqData.putField("XCL_DEALCO_CD", "");
                        // 정산등록
                        dEPCnclInveRevdSlipRgst.dICnclInveAccoRmtXcl(reqData, onlineCtx);
