...
//				logger.debug("\n\n\n\n\n   >>>>>>>>>>>>>>>>>   "+requestData.getField("IN_GUBUN"));
				...
				if("NO".equals(requestData.getField("IN_GUBUN")) || "%".equals(requestData.getField("IN_GUBUN")) ){
...
//					logger.debug("\n mProdDtl:"+mProdDtl);
				...
				mMgmt.put("ACCO_NO", HpcUtils.maskingAccountNo(mMgmt.get("ACCO_NO").toString()));
...
//				logger.debug("\n\n userid:"+requestData.getField("USERID")+"\n\n");
	...
	/**
	 * <pre>[DM]입고대상상품상세리스트</pre>
	 *
	 * @author 김상오 (myneti99)
	 * @since 2015-11-13 10:05:38
	 *
	 * @param requestData 요청정보 DataSet 객체
	 * @param onlineCtx   요청 컨텍스트 정보
	 * @return 처리결과 DataSet 객체
	 */
	public IDataSet dSInqEqpProdDtlList(IDataSet requestData, IOnlineContext onlineCtx) {
		IDataSet responseData = new DataSet();
	    try {       
	    	IDataSet reqDs = (IDataSet) requestData.clone();//원거래의 DataSet의 훼손을 막기 위한 Clone

	    	// 1.쿼리문 호출
	    	IRecordSet rs  = dbSelect("SInqEqpProdDtlList", reqDs, onlineCtx);
	    	// 2.결과값 셋팅
            responseData.putRecordSet("RS_EQP_PROD_DTL_LIST", rs);
	 ...
	 * @return 처리결과 DataSet 객체
	 */
	public IDataSet dSAssetInfo(IDataSet requestData, IOnlineContext onlineCtx) {
		IDataSet responseData = new DataSet();
	    try {       
	    	IDataSet reqDs = (IDataSet) requestData.clone();//원거래의 DataSet의 훼손을 막기 위한 Clone

	    	// 1.쿼리문 호출
	    	IRecordSet rs  = dbSelect("SAssetInfo", reqDs, onlineCtx);
	    	// 2.결과값 셋팅
            responseData.putRecordSet("RS_ASSET_INFO", rs);
	 ...
	 *
	 * @author 김상오 (myneti99)
	 * @since 2015-11-18 09:26:24
	 *
	 * @param requestData 요청정보 DataSet 객체
	 * @param onlineCtx   요청 컨텍스트 정보
	 * @return 처리결과 DataSet 객체
	 */
	public IDataSet dICnslMgmtDtl(IDataSet requestData, IOnlineContext onlineCtx) {
	    IDataSet responseData = new DataSet();
	
	    dbInsert("DEPEqpInMgmt.ICnslMgmtDtl", requestData, onlineCtx);   //("CSTADV00600.addConsultDtlXls", mProdDtl, onlineCtx); 
	 ...
	 *
	 * @author 김상오 (myneti99)
	 * @since 2015-11-18 09:53:37
	 *
	 * @param requestData 요청정보 DataSet 객체
	 * @param onlineCtx   요청 컨텍스트 정보
	 * @return 처리결과 DataSet 객체
	 */
	public IDataSet dICnslMgmt(IDataSet requestData, IOnlineContext onlineCtx) {
		IDataSet responseData = new DataSet();
		
	    dbInsert("DEPEqpInMgmt.ICnslMgmt", requestData, onlineCtx);   //("CSTADV00600.addConsultDtlXls", mProdDtl, onlineCtx); 
