...
public class FEPRtnMgmt extends fwk.base.FunctionUnit {
	...
	public IDataSet fRegJdgRtn(IDataSet requestData, IOnlineContext onlineCtx) {
        ...
        try {
            ...
            // 2. 반제전표용자료생성
            /*--자동반제처리 사용 안하기로함
            if(requestData != null && requestData.getRecordSet("RQ_JDG_RTN_CON_LIST") != null && requestData.getRecordSet("RQ_JDG_RTN_CON_LIST").getRecordCount() > 0){
                FEPCnclInveRevdSlipRgst fEPCnclInveRevdSlipRgst = (FEPCnclInveRevdSlipRgst) lookupFunctionUnit(FEPCnclInveRevdSlipRgst.class);
                rqRevdData.putRecordSet("RQ_RVED_SLIP_LIST",requestData.getRecordSet("RQ_JDG_RTN_CON_LIST"));
	...
	public IDataSet fRegRtnRegObjList(IDataSet requestData, IOnlineContext onlineCtx) {
        ...
        try {
            ...
            // 2. 반제전표용자료생성
            /*--자동반제처리 사용 안하기로함
    		if(requestData != null && requestData.getRecordSet("RQ_AGN_RTN_LIST") != null && requestData.getRecordSet("RQ_AGN_RTN_LIST").getRecordCount() > 0){
        		FEPCnclInveRevdSlipRgst fEPCnclInveRevdSlipRgst = (FEPCnclInveRevdSlipRgst) lookupFunctionUnit(FEPCnclInveRevdSlipRgst.class);
        		rqRevdData.putRecordSet("RQ_RVED_SLIP_LIST",requestData.getRecordSet("RQ_AGN_RTN_LIST"));
        		String strChk = "";
        		for(int i = (rqRevdData.getRecordSet("RQ_RVED_SLIP_LIST").getRecordCount()-1); i >= 0; i--){
        		    strChk = rqRevdData.getRecordSet("RQ_RVED_SLIP_LIST").getRecord(i).get("CHK").toString();
        		    if(strChk == null || "".equals(strChk) || "0".equals(strChk)){
