...
public class PSCLginOp extends fwk.base.ProcessUnit {
	...
	public IDataSet pLgin(IDataSet requestData, IOnlineContext onlineCtx) {
		...
		try {
			...
			*********************************************************************************************/
            ...
            // 사용자 정보 객체 생성 및 정보 설정
            UserInfo userInfo = new UserInfo();
            // 기본 항목 설정
            userInfo.setLoginId(userRs.get(0, "LGIN_ID"));
            userInfo.setLoginTime(new Date());
            userInfo.getLocale(ca.getUserLocale());
              
            // 생성한 사용자 정보 객체를 응답데이타에 설정하여 리턴한다.
            responseData.putField(Constants.USER, userInfo);
	...
	public IDataSet pLgout(IDataSet requestData, IOnlineContext onlineCtx) {
		...
		try {
		    ...
		    if ( StringUtils.equals(getCommonArea(onlineCtx).getFrstTrnmChnlCd(), "XIUI")) {
                ...
                requestData.putField("USER_NO", onlineCtx.getUserInfo().get("USER_NO"));
                requestData.putField("SESS_NO", onlineCtx.getUserInfo().get("SESS_NO"));
            }
	 ...
	 * </pre>
	 */
	public IDataSet pLginWeb(IDataSet requestData, IOnlineContext onlineCtx) {
        ...
        try {
            ...
            *********************************************************************************************/
            ...
            requestData.putField("SESS_NO", sessionKey);
            requestData.putField("LGIN_DTM", userRs.get(0, "LS_LGIN_DTM"));
            requestData.putField("IP_ADDR", onlineCtx.getUserInfo().getIp());
            requestData.putField("FS_REG_USER_ID", userRs.get(0, "USER_NO"));
            requestData.putField("LS_CHG_USER_ID", userRs.get(0, "USER_NO"));//
            ...
            userInfo.getLocale(ca.getUserLocale());
            // 추가 항목 설정
            userInfo.put("USER_NO", userRs.get(0, "USER_NO"));
            userInfo.put("USER_NM", userRs.get(0, "USER_NM"));
            userInfo.put("SESS_NO", sessionKey);
            //userInfo.put("ROLE", "");
            
            //3.5.1. 대리점정보 조회
            String dealCoCd = userRs.get(0, "DEALCO_CD");
            userInfo.put("DEALCO_CD", dealCoCd); //대리점코드
           
            IDataSet dealRsData = new DataSet();
            IRecordSet dealRs = null;
         
            if(StringUtils.isNotEmpty(dealCoCd)){
                  requestData.putField("DEALCO_CD", dealCoCd);  
                  
                  FSCDealMgmt fSCDealMgmt = (FSCDealMgmt) lookupFunctionUnit(FSCDealMgmt.class);
                  //대리점정보조회 FM 호출
                  dealRsData = fSCDealMgmt.fInqDealInfo(requestData, onlineCtx);
                  dealRs = dealRsData.getRecordSet("RS_DEAL_INFO");
                  
                  if(dealRs.getRecordCount() >0){
                      String dealcoRepveNm = StringUtils.nvlObject(dealRs.get(0, "DEALCO_REPVE_NM"), "");
                      if(StringUtils.isNotEmpty(dealcoRepveNm)){
                          dealcoRepveNm = HpcUtils.decodeByAES(dealcoRepveNm);
                      }
                      userInfo.put("DEALCO_NM", dealRs.get(0, "DEALCO_NM")); //대리점명
                      userInfo.put("DEALCO_REPVE_NM", dealcoRepveNm ); //대표자명 
                      userInfo.put("AGN_ORG_CD",  dealRs.get(0, "AGN_ORG_CD")); //조직코드 
                      userInfo.put("DEALCO_BLICENS_NO", dealRs.get(0, "DEALCO_BLICENS_NO")); //사업자번호
                  }
