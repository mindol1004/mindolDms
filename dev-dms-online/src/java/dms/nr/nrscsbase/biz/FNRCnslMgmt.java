...
public class FNRCnslMgmt extends fwk.base.FunctionUnit {
	...
	public IDataSet fInqScrbPrstDetl(IDataSet requestData, IOnlineContext onlineCtx) {
        ...
        try {
            ...
            DNRCnslMgmt dNRCnslMgmt = (DNRCnslMgmt) lookupDataUnit(DNRCnslMgmt.class);
            // 3. LISTDM호출 (RS_MAIN)
            responseData = dNRCnslMgmt.dSScrbPrstDetl(reqDs, onlineCtx);
            ...
            //정책정보조회  조회
			IDataSet dsRtn = dNRCnslMgmt.dSScrbPrstPenDetl(reqDs, onlineCtx);
	 ...
	 * @return 처리결과 DataSet 객체
	 */
    public IDataSet fRegRentalCnslNEW(IDataSet requestData, IOnlineContext onlineCtx) {
        IDataSet responseData = new DataSet();
        try {
            // 1. DU lookup
            DNRCnslMgmt dNRCnslMgmt = (DNRCnslMgmt) lookupDataUnit(DNRCnslMgmt.class);
            ...
            requestData.putField("CNSL_NO", valDS.getField("CNSL_NO"));
            // 3. 업무 DM호출
            responseData = dNRCnslMgmt.dIRentalCnslNEW(requestData, onlineCtx);
        } catch ( BizRuntimeException e ) {
            throw e;
        }
        return responseData;
	 ...
	 * @return 처리결과 DataSet 객체
	 */
    public IDataSet fUpdRentalCnsl(IDataSet requestData, IOnlineContext onlineCtx) {
        IDataSet responseData = new DataSet();
        try {
            // 1. DU lookup
            DNRCnslMgmt dNRCnslMgmt = (DNRCnslMgmt) lookupDataUnit(DNRCnslMgmt.class);
            
            responseData = dNRCnslMgmt.dURentalCnsl(requestData, onlineCtx);
        } catch ( BizRuntimeException e ) {
            throw e;
        }
        return responseData;
	 ...
	 * </pre>
	 */
    public IDataSet fInqEqpInsuCnslLst(IDataSet requestData, IOnlineContext onlineCtx) {
        IDataSet responseData = new DataSet();
        IDataSet dsCnt = new DataSet();
        IRecordSet rsPagingList = null;
        int intTotalCnt = 0;
        ...
        try {
            ...
            DNRCnslMgmt dNRCnslMgmt = (DNRCnslMgmt) lookupDataUnit(DNRCnslMgmt.class);
            // 2. TOTAL COUNT DM호출
            dsCnt = dNRCnslMgmt.dSEqpInsuCnslTotCnt(requestData, onlineCtx);
            intTotalCnt = Integer.parseInt(dsCnt.getField("TOTAL_CNT"));
            
            PagenateUtils.setPagenatedParamsToDataSet(requestData);
            // 3. LISTDM호출
            responseData = dNRCnslMgmt.dSEqpInsuCnslPaging(requestData, onlineCtx);
            rsPagingList = responseData.getRecordSet("RS_MAIN");
	 ...
	 * @return 처리결과 DataSet 객체
	 */
    public IDataSet fRegEqpInsuCnsl(IDataSet requestData, IOnlineContext onlineCtx) {
        IDataSet responseData = new DataSet();
        try {
            // 1. DU lookup
            DNRCnslMgmt dNRCnslMgmt = (DNRCnslMgmt) lookupDataUnit(DNRCnslMgmt.class);
            IDataSet valDS = dNRCnslMgmt.dSEqpInsuCnslNo(requestData, onlineCtx);
            requestData.putField("CNSL_NO", valDS.getField("CNSL_NO"));
            // 3. 업무 DM호출
            responseData = dNRCnslMgmt.dIEqpInsuCnsl(requestData, onlineCtx);
        } catch ( BizRuntimeException e ) {
            throw e;
        }
        return responseData;
	 ...
	 * @return 처리결과 DataSet 객체
	 */
    public IDataSet fRegEqpInsuCnslNEW(IDataSet requestData, IOnlineContext onlineCtx) {
        IDataSet responseData = new DataSet();
        try {
            // 1. DU lookup
            DNRCnslMgmt dNRCnslMgmt = (DNRCnslMgmt) lookupDataUnit(DNRCnslMgmt.class);
            ...
            requestData.putField("CNSL_NO", valDS.getField("CNSL_NO"));
            // 3. 업무 DM호출
            responseData = dNRCnslMgmt.dIEqpInsuCnslNEW(requestData, onlineCtx);
        } catch ( BizRuntimeException e ) {
            throw e;
        }
        return responseData;
	 ...
	 * @return 처리결과 DataSet 객체
	 */
    public IDataSet fUpdEqpInsuCnsl(IDataSet requestData, IOnlineContext onlineCtx) {
        IDataSet responseData = new DataSet();
        try {
            // 1. DU lookup
            DNRCnslMgmt dNRCnslMgmt = (DNRCnslMgmt) lookupDataUnit(DNRCnslMgmt.class);
            
            responseData = dNRCnslMgmt.dUEqpInsuCnsl(requestData, onlineCtx);
        } catch ( BizRuntimeException e ) {
            throw e;
        }
        return responseData;
