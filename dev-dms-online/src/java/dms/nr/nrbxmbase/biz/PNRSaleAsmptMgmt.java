...
public class PNRSaleAsmptMgmt extends fwk.base.ProcessUnit {
	 ...
	 *		- field : SALE_DEALCO_FISCL_ST_CD [매출거래처회계상태코드]
	 * </pre>
	 */
	 ...
	 *		- field : SALE_ASMPT_YM [년월]
	 *		- field : YYYYMM [필드1]
	 *		- field : SALE_DEALCO_FISCL_ST_CD [필드1]
	 * </pre>
	 * @param onlineCtx   요청 컨텍스트 정보
	 * @return 처리결과 DataSet 객체
	 */
	 ...
	 * @return 처리결과 DataSet 객체
	 */
	public IDataSet pInqSaleSecondReBatch(IDataSet requestData, IOnlineContext onlineCtx) {
		 IDataSet responseData = new DataSet();
			
		    try {
				// 1. FM 호출
				responseData = callSharedBizComponentByDirect("nr.NRSXMBase", "fInqSaleSecondReBatch", requestData, onlineCtx);
			} catch ( BizRuntimeException e ) {
				throw e;
			} catch ( Exception e ) {
				throw new BizRuntimeException("DMS00009", e); // 시스템 오류가 발생하였습니다.
