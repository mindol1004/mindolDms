 ...
 * <li>단위업무명: [PU]자산정산등록</li>
 * <li>설  명 : <pre>[PU]자산정산등록</pre></li>
 ...
 * @author 김상오 (myneti99)
 */
public class PPRAssetSaleRgst extends fwk.base.ProcessUnit {
	 ...
	 * </pre>
	 */
	public IDataSet pInqAssetSaleList(IDataSet requestData, IOnlineContext onlineCtx) {
	 ...
	 * @return 처리결과 DataSet 객체
	 */
	public IDataSet pSaveDeprXcl(IDataSet requestData, IOnlineContext onlineCtx) {
	    IDataSet responseData = new DataSet();
	    Log logger = LogManager.getFwkLog(); 
	...
	//	    }
	    ...
	    inParam.put("LGIN_ID", onlineCtx.getUserInfo().getLoginId());
	 ...
	 * </pre>
	 */
	public IDataSet pInqAssetSaleExcelList(IDataSet requestData, IOnlineContext onlineCtx) {
	    IDataSet responseData = new DataSet();
	//	    Log logger = LogManager.getFwkLog(); 
	//	    logger.debug("\n\n\n  >>>>>>>>> fieldMap : "+ requestData.getFieldMap());
	    //FM 호출
	    responseData = callSharedBizComponentByDirect("pr.PRSXMBase", "fInqAssetDeprExcelList", requestData, onlineCtx);
	    //결과값 리턴
	    responseData.setOkResultMessage("DMS00001", null); //정상 조회되었습니다.
