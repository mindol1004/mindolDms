 ...
 * @author 이준우 (elmsliw)
 */
public class FPREqpInveLst extends fwk.base.FunctionUnit {
	 ...
	 * </pre>
	 */
    public IDataSet fInqEqpInveLst(IDataSet requestData, IOnlineContext onlineCtx) {
    	...
    	try {
    		// 1. DU lookup
    	    DPREqpInveLst dPREqpInveLst = (DPREqpInveLst) lookupDataUnit(DPREqpInveLst.class);
    		
    		// 2. TOTAL COUNT DM호출
    		dsCnt = dPREqpInveLst.dSEqpInveTotCnt(reqDs, onlineCtx);
    		intTotalCnt = Integer.parseInt(dsCnt.getField("TOTAL_CNT"));  
    		PagenateUtils.setPagenatedParamsToDataSet(reqDs);	//카운트 셋
    		
    		// 3. LIST DM호출
    		responseData = dPREqpInveLst.dSEqpInveLstPaging(reqDs, onlineCtx);
    
    		rsPagingList = responseData.getRecordSet("RS_RIM_LIST");
