...
public class DPREqpReqpInMgmt extends fwk.base.DataUnit {
	 ...
	 *
	 * @author 김상오 (myneti99)
	 * @since 2015-10-19 14:23:06
	 *
	 * @param requestData 요청정보 DataSet 객체
	 * @param onlineCtx   요청 컨텍스트 정보
	 * @return 처리결과 DataSet 객체
	 */
	public IDataSet dIEqpInPrch(IDataSet requestData, IOnlineContext onlineCtx) {
	    IDataSet responseData = new DataSet();
	    Log logger = LogManager.getFwkLog();
	    logger.debug("\n\n\n\n 매입등록  LS_CHG_USER_ID :"+requestData.getField("LS_CHG_USER_ID"));
	    logger.debug("\n\n\n\n 매입등록  requestData :"+requestData.getFieldMap());
	    
	    // 1.쿼리문 호출
	    String prchNo = (String)dbInsertAndReturnPK("IEqpInPrch", requestData, onlineCtx);
	    logger.debug("\n\n\n\n >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>                        prchNo : "+prchNo);
	    responseData.putField("PRCH_NO", prchNo);
	 	return responseData;
	}
}
