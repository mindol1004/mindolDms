 ...
 * <li>설  명 : <pre>[FU]손해배상금정책 관리 </pre></li>
 ...
 */
public class FPRCmpPolyMgmt extends fwk.base.FunctionUnit {
	...
	public IDataSet fInqCmpPolyLst(IDataSet requestData, IOnlineContext onlineCtx) {
		...
		try {
			// 1. DU lookup
			DPRCmpPolyMgmt dPRCmpPolyMgmt = (DPRCmpPolyMgmt) lookupDataUnit(DPRCmpPolyMgmt.class);
			
			// 2. TOTAL COUNT DM호출
			dsCnt = dPRCmpPolyMgmt.dSCmpPolyTotCnt(requestData, onlineCtx);
			intTotalCnt = Integer.parseInt(dsCnt.getField("TOTAL_CNT"));
			PagenateUtils.setPagenatedParamsToDataSet(reqDs);
	
			// 3. LISTDM호출
			responseData = dPRCmpPolyMgmt.dSCmpPolyLstPaging(reqDs, onlineCtx);
	...
	public IDataSet fRegCmpPoly(IDataSet requestData, IOnlineContext onlineCtx) {
	    ...
	    try {	    	
	        ...
	        // 유효시작일 중복 체크
			IDataSet dsRtn = dPRCmpPolyMgmt.dSChkDupFrDt(requestData, onlineCtx);
			...
			// 2. 손해배상금정책 채번
			String DCP_NO = dPRCmpPolyMgmt.dSCmpPolyNo(requestData, onlineCtx).getField("DCP_NO");
			String USER_NO = requestData.getField("USER_NO");
			
			// 3. 업무 DM호출
			// 손해배상금 등록
			requestData.putField("DCP_NO", DCP_NO);			
			responseData = dPRCmpPolyMgmt.dICmpPoly(requestData, onlineCtx);
			...
			for(int idx=0; idx<rs.getRecordCount(); ++idx){
				...
				responseData = dPRCmpPolyMgmt.dICmpPolyDtl(reqDs, onlineCtx);
			}
			
	        //이전정책종료일 변경
	        responseData = dPRCmpPolyMgmt.dUPreCmpPoly(requestData, onlineCtx);
	...
	public IDataSet fUpdCmpPoly(IDataSet requestData, IOnlineContext onlineCtx) {
	    ...
	    try {
			...
			// 유효시작일 중복 체크
	        IDataSet dsRtn = dPRCmpPolyMgmt.dSChkDupFrDt2(requestData, onlineCtx);
	        if( dsRtn.getIntField("DUP_CNT") > 0){
			...
			// 3. 업무 DM호출
			responseData = dPRCmpPolyMgmt.dUCmpPoly(requestData, onlineCtx);
	        //점검항목등록
	        String USER_NO = requestData.getField("USER_NO");
	        IRecordSet rs = requestData.getRecordSet("RS_CMP_POLY_DTL");           
	        for(int idx=0; idx<rs.getRecordCount(); ++idx){
	            ...
	            reqDs.putFieldMap( rs.getRecord(idx) );
	            reqDs.putField("USER_NO", USER_NO);
	            responseData = dPRCmpPolyMgmt.dUCmpPolyDtl(reqDs, onlineCtx);
	        }
	...
	public IDataSet fInqEqpConsPen(IDataSet requestData, IOnlineContext onlineCtx) {
		...
		try {
			...
			// 2. LISTDM호출
			responseData = dPRCmpPolyMgmt.dSEqpConsPen(reqDs, onlineCtx);
	...
	public IDataSet fInqCmpPolyDtl(IDataSet requestData, IOnlineContext onlineCtx) {
		...
		try {
			...
			// 2. LISTDM호출
			responseData = dPRCmpPolyMgmt.dSCmpPolyDtl(requestData, onlineCtx);
	...
	public IDataSet fDelCmpPoly(IDataSet requestData, IOnlineContext onlineCtx) {
	    ...
	    try {            
	        ...
	        if( Integer.parseInt(DateUtils.getCurrentDate("yyyyMMdd")) >= Integer.parseInt(rs.get(0, "DCP_EFF_PRD_STA_DT")) ){
	            throw new BizRuntimeException("DMS00025"); //삭제할 수 없는 데이터 입니다.
	        };
	        
	        // 2. 삭제
	        responseData = dPRCmpPolyMgmt.dDCmpPolyDtl(requestData, onlineCtx);            
	        responseData = dPRCmpPolyMgmt.dDCmpPoly(requestData, onlineCtx);
