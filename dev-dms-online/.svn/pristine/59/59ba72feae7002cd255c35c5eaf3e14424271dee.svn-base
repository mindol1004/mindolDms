...
//            responseData.putRecordSet("RS_CMPHS_CNTRT_LIST",fPRRentalServCntrtMgmt.fInqCmphsCntrtLst(requestData, onlineCtx).getRecordSet("RS_CMPHS_CNTRT_LIST"));
            responseData = callSharedBizComponentByDirect("pr.PRSXMBase", "fInqCmphsCntrtLst", requestData, onlineCtx);
            // 3. 결과값 리턴
...
//            responseData.putRecordSet("RS_EQP_CNTRT_LIST",fPRRentalServCntrtMgmt.fInqEqpCntrtLst(requestData, onlineCtx).getRecordSet("RS_EQP_CNTRT_LIST"));
            responseData = callSharedBizComponentByDirect("pr.PRSXMBase", "fInqEqpCntrtLst", requestData, onlineCtx);
            // 3. 결과값 리턴
            responseData.setOkResultMessage("DMS00001", null); //정상 조회되었습니다.
        } catch ( BizRuntimeException e ) {
            throw e;
...
//                fPRRentalServCntrtMgmt.fRgsCmphsCntrt(requestData, onlineCtx);
                responseData = callSharedBizComponentByDirect("pr.PRSXMBase", "fRgsCmphsCntrt", requestData, onlineCtx);
            } else if ( "U".equals(ca.getTrnPtrnDvcd()) ) { // UPDATE
//                fPRRentalServCntrtMgmt.fUpdCmphsCntrt(requestData, onlineCtx);
                responseData = callSharedBizComponentByDirect("pr.PRSXMBase", "fUpdCmphsCntrt", requestData, onlineCtx);
            } else if ( "D".equals(ca.getTrnPtrnDvcd()) ) { // DELETE
//                fPRRentalServCntrtMgmt.fDelCmphsCntrt(requestData, onlineCtx);
                responseData = callSharedBizComponentByDirect("pr.PRSXMBase", "fDelCmphsCntrt", requestData, onlineCtx);
            }
            // 4. 결과값 리턴
...
//            FPRRentalServCntrtMgmt fPRRentalServCntrtMgmt = (FPRRentalServCntrtMgmt) lookupFunctionUnit(FPRRentalServCntrtMgmt.class);
            ...
            requestData.putFieldMap(requestData.getRecordSet("RQ_EQP_CNTRT_CMPT_LIST").getRecordMap(0));
            // 3. 레코드셋 상태에 따른 분기
            if ( "I".equals(ca.getTrnPtrnDvcd()) ) { // INSERT
//                fPRRentalServCntrtMgmt.fRgsEqpRentalCntrt(requestData, onlineCtx);
                responseData = callSharedBizComponentByDirect("pr.PRSXMBase", "fRgsEqpRentalCntrt", requestData, onlineCtx);
            } else if ( "U".equals(ca.getTrnPtrnDvcd()) ) { // UPDATE
//                fPRRentalServCntrtMgmt.fUpdEqpRentalCntrt(requestData, onlineCtx);
                responseData = callSharedBizComponentByDirect("pr.PRSXMBase", "fUpdEqpRentalCntrt", requestData, onlineCtx);
                //구성품처리
//                fPRRentalServCntrtMgmt.fUpdEqpCntrtCmptUprcMgmt(requestData, onlineCtx);
                responseData = callSharedBizComponentByDirect("pr.PRSXMBase", "fUpdEqpCntrtCmptUprcMgmt", requestData, onlineCtx);
            } else if ( "D".equals(ca.getTrnPtrnDvcd()) ) { // DELETE
//                fPRRentalServCntrtMgmt.fDelEqpRentalCntrt(requestData, onlineCtx);
                responseData = callSharedBizComponentByDirect("pr.PRSXMBase", "fDelEqpRentalCntrt", requestData, onlineCtx);
            } 
            // 4. 결과값 리턴
            responseData.setOkResultMessage("DMS00000", null); //정상 처리되었습니다.
        } catch ( BizRuntimeException e ) {
...
//            responseData.putRecordSet("RS_EQP_CNTRT_CMPT_UPRC_LIST",fPRRentalServCntrtMgmt.fInqEqpCntrtCmptUprcLst(requestData, onlineCtx).getRecordSet("RS_EQP_CNTRT_CMPT_UPRC_LIST"));
            responseData = callSharedBizComponentByDirect("pr.PRSXMBase", "fInqEqpCntrtCmptUprcLst", requestData, onlineCtx);
            // 3. 결과값 리턴
            responseData.setOkResultMessage("DMS00001", null); //정상 조회되었습니다.
        } catch ( BizRuntimeException e ) {
            throw e;
	 ...
	 * </pre>
	 */
	public IDataSet pInqRentRCntrtLst(IDataSet requestData, IOnlineContext onlineCtx) {
        IDataSet responseData = new DataSet();
...
//            FPRRentalServCntrtMgmt fPRRentalServCntrtMgmt = (FPRRentalServCntrtMgmt) lookupFunctionUnit(FPRRentalServCntrtMgmt.class);
            // 2. FM 호출
//            responseData.putRecordSet("RENTR_CNRTR_LIST",fPRRentalServCntrtMgmt.fInqRentRCntrtLst(requestData, onlineCtx).getRecordSet("RENTR_CNRTR_LIST"));
        	responseData = callSharedBizComponentByDirect("pr.PRSXMBase", "fInqRentRCntrtLst", requestData, onlineCtx);
            // 3. 결과값 리턴
            responseData.setOkResultMessage("DMS00001", null); //정상 조회되었습니다.
        } catch ( BizRuntimeException e ) {
            throw e;
