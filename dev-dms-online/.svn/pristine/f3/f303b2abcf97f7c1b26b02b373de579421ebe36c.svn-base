...
public class FSCPrnInfoInq extends fwk.base.FunctionUnit {
	...
	public IDataSet fInqPrnInfo(IDataSet requestData, IOnlineContext onlineCtx) {
        ...
        try {
            ...
            for ( int i = 0 ; i < reqCnt ; i++ ) {
                reqData.putFieldMap(reqRecordSet.getRecord(i));
                prnInfoInqItemCd = reqData.getField("PRN_INFO_INQ_ITEM_CD");
                prnInfoInqCtt = reqData.getField("PRN_INFO_INQ_CTT");
                ...
                if ( StringUtils.isEmpty(reqData.getField("INQ_OBJ_NO")) ) {
                    throw new BizRuntimeException("DMS00002", new String[] { HpcUtils.getLangMsg("INQ_OBJ_NO") });
                }
                
                //개인정보내용 복호화 
                try {
                    prnInfoInqCtt = HpcUtils.decodeByAES(prnInfoInqCtt);
                } catch ( Exception e ) {
                ...
                // 개인정보항목코드  (DMS007:개인정보항목코드)
                record.set("PRN_INFO_INQ_ITEM_CD", prnInfoInqItemCd);
                record.set("PRN_INFO_INQ_ITEM_NM", HpcUtils.getCode("DMS007", prnInfoInqItemCd).getCodeNm());
                record.set("PRN_INFO_INQ_CTT", prnInfoInqCtt);
                
                if ( bSuccess && StringUtils.equals(reqData.getField("REG_HST_YN"), "Y")) {
                    // 개인정보조회이력 등록
	...
	/**
	 * <pre>개인정보조회 화면조회용 서비스
	 *이메일,전화번호 등 여러종류의 개인정보를 한꺼번에 조회해오기 위한 용도이다.</pre>
	 *
	 * @author admin (admin)
	 * @since 2015-07-13 12:41:50
	 *
	...
	public IDataSet fInqPrnInfoNoRegHst(IDataSet requestData, IOnlineContext onlineCtx) {
		...
		try {
			...
			for ( int i = 0 ; i < reqCnt ; i++ ) {
				...
				rsRecord.set("PRN_INFO_INQ_ITEM_NM", HpcUtils.getCode("DMS007", prnInfoInqItemCd).getCodeNm());
