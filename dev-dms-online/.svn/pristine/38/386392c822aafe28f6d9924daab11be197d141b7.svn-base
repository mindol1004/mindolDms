...
public class FEPFPAProxySaleMgmt extends fwk.base.FunctionUnit {
                ...
                for (int i = 0; i < excelRs.getRecordCount(); i++) {
                    ...
                    */
                    ...
                    if (disMap.containsKey(excelRd.get("EQP_MDL_CD")+excelRd.get("PROD_SER_NO"))) {
                        ...
                        excelRd.set("MQ_SELL_PRC", value.get("MQ_SELL_PRC")); //MQ판매가
	 ...
	 * @return 처리결과 DataSet 객체
	 */
	public IDataSet fInqFpaProxySaleCnclList(IDataSet requestData, IOnlineContext onlineCtx) {
	    IDataSet responseData = new DataSet();
        IDataSet dsCnt = new DataSet();
        IDataSet reqDs = (IDataSet) requestData.clone();//원거래의 DataSet의 훼손을 막기 위한 Clone
        IRecordSet rsPagingList = null;
        ...
        try {
            ...
            DEPFPAProxySaleMgmt dEPFPAProxySaleMgmt = (DEPFPAProxySaleMgmt) lookupDataUnit(DEPFPAProxySaleMgmt.class);
            // 2. TOTAL COUNT DM호출
            dsCnt = dEPFPAProxySaleMgmt.dSFpaProxySaleCnclListTotCnt(reqDs, onlineCtx);
            intTotalCnt = Integer.parseInt(dsCnt.getField("TOTAL_CNT"));
            PagenateUtils.setPagenatedParamsToDataSet(reqDs);
            // 3. LISTDM호출
            responseData = dEPFPAProxySaleMgmt.dSFpaProxySaleCnclListPaging(reqDs, onlineCtx);
            rsPagingList = responseData.getRecordSet("RS_FPA_PROXY_CNCL_LIST");
            PagenateUtils.setPagenatedParamToRecordSet(rsPagingList, reqDs, intTotalCnt);
	 ...
	 * @return 처리결과 DataSet 객체
	 */
	public IDataSet fRegFpaProxySaleCncl(IDataSet requestData, IOnlineContext onlineCtx) {
	    CommonArea ca = getCommonArea(onlineCtx);
        IDataSet responseData = new DataSet();
        IDataSet reqDs = (IDataSet) requestData.clone();//원거래의 DataSet의 훼손을 막기 위한 Clone
        ...
        try {
            ...
            if (saleRs != null) {
                ...
                for (int i = 0; i < saleRs.getRecordCount(); i++) {
                    ...
                    dsCnt = dEPFPAProxySaleMgmt.dSFpaProxyGnrlSaleCount(saleDs, onlineCtx);
                    ...
                    seqDs = dEPFPAProxySaleMgmt.dSFpaProxySaleGnrlChgNo(saleDs, onlineCtx);
                    ...
                    dEPFPAProxySaleMgmt.dIFpaProxyGnrlSaleCncl(saleDs, onlineCtx); // 일반판매저장
                    dEPFPAProxySaleMgmt.dIFpaProxySaleEquipmentCncl(saleDs, onlineCtx); // 단말기판매저장
                ...
                for (int i = 0; i < saleRs.getRecordCount(); i++) {
                    ...
                    dEPFPAProxySaleMgmt.dUFpaProxySaleEquipmentCncl(saleDs, onlineCtx); // 단말기판매취소수정
                }
