...
public class PPREqpReqpInMgmt extends fwk.base.ProcessUnit {
	...
	public IDataSet pSaveEqpReqpIn(IDataSet requestData, IOnlineContext onlineCtx) {
        ...
        IRecordSet rsCmptOut = null;
        IRecordSet rsCmptOutTemp = null;

		try {
	        if ( "I".equals(ca.getTrnPtrnDvcd()) ) { // INSERT
	            rsCmptOut = requestData.getRecordSet("CMPT_OUT");
	        }
			// 1. 레코드셋 상태에 따른 분기
			for (int i=0; i<rs.getRecordCount(); i++) {
				...
				if(reqDate.getField("DEL_YN") == null) {
				    reqDate.putField("DEL_YN", "N");
				}
				if(reqDate.getField("OP_CL_CD") == null) {
				    reqDate.putField("OP_CL_CD", "PR");
				}
				if(reqDate.getField("INVE_ST_CD") == null) {
				    reqDate.putField("INVE_ST_CD", "10");
				}
                if(reqDate.getField("IN_DTL_CD") == null) {
                    reqDate.putField("IN_DTL_CD", "10");
                }
				
				if ( "I".equals(ca.getTrnPtrnDvcd()) ) { // INSERT
	                IRecord record = null;
	                rsCmptOutTemp = new RecordSet("CMPT_OUT", new String[]{"PAR_NO","CHECKED","CMPT_CD","CMPT_USE_QTY","OUT_DTL_CD"});
	                for(int j = 0; j < rsCmptOut.getRecordCount(); j++){
	                    rsCmpt.putFieldMap(rsCmptOut.getRecordMap(j));
	                    if(!StringUtils.isEmpty(reqDate.getField("PAR_NO")) && !StringUtils.isEmpty(rsCmpt.getField("PAR_NO")) 
	                            && reqDate.getField("PAR_NO").toString().equals(rsCmpt.getField("PAR_NO").toString())
	                            && !StringUtils.isEmpty(rsCmpt.getField("CHECKED"))
	                            && "1".equals(rsCmpt.getField("CHECKED").toString())){
	                        record = rsCmptOutTemp.newRecord();
	                        record.set("PAR_NO", rsCmpt.getField("PAR_NO"));
	                        record.set("CHECKED", rsCmpt.getField("CHECKED"));
	                        record.set("CMPT_CD", rsCmpt.getField("CMPT_CD"));
	                        record.set("CMPT_USE_QTY", rsCmpt.getField("CMPT_USE_QTY"));
	                        record.set("OUT_DTL_CD", rsCmpt.getField("OUT_DTL_CD"));
	                        rsCmptOutTemp.addRecord(record);
	                    }
	                }
	                reqDate.putRecordSet("CMPT_OUT", rsCmptOutTemp);

	                callSharedBizComponentByDirect("pr.PRSPMBase", "fRegEqpReqpIn", reqDate, onlineCtx);
				} else if ( "D".equals(ca.getTrnPtrnDvcd()) ) { // DELETE
					callSharedBizComponentByDirect("pr.PRSPMBase", "fDelInEqpLst", reqDate, onlineCtx);
				} 
			}
	...
	public IDataSet pInqEqpReqpInDtlLst(IDataSet requestData, IOnlineContext onlineCtx) {
	    ...
	    try {
            ...
            if(rsCmpt != null && responseData != null && rs.getRecordCount() > 0){
