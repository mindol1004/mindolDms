...
public class PSCAnncBrdMgmt extends fwk.base.ProcessUnit {
	...
	public IDataSet pSAnncBrdForWebNotice(IDataSet requestData, IOnlineContext onlineCtx) {
        ...
        try {
            ...
            if ( StringUtils.equals(requestData.getField("LIST_YN"), "Y") ) {
                ...
                tempRS = responseDataTemp.getRecordSet("RS_ANNCE");
                // 웹에서 접근 불가능한 게시물 필터링
                if (!(StringUtils.equals("30", tempRS.getRecord(0).get("ANNCE_TYP")) || StringUtils.equals("40", tempRS.getRecord(0).get("ANNCE_TYP")))){
                    throw new BizRuntimeException("DMS00157"); // 처리할 데이터가 없습니다.
                }
                responseData.putRecordSet("RS_ANNCE", tempRS);
                
                //requestData에 조회조건 파라미터로 추가
                requestData.putFieldMap(tempRS.getRecordMap(0));
                 // 2. 조회수 증가
                fSCAnncBrdMgmt.fUpdAnncIncreBrwsCnt(requestData, onlineCtx);
                // 3. 업로드파일 조회
                responseDataTemp= callSharedBizComponentByDirect("sc.SCSBase", "fInqUpFileInfo", requestData, onlineCtx);                
    ...
    /**
	 * <pre>DMS 메인화면에 보여지는 공지사항목록 및 팝업목록을 조회한다.</pre>
	 *
	 * @author 박홍서 (uni9401)
	 * @since 2015-06-29 10:33:43
	 *
	 ...
	 * </pre>
	 */
	public IDataSet pInqAnncBrdLstForDMS(IDataSet requestData, IOnlineContext onlineCtx) {
	    IDataSet responseData = new DataSet();
        IDataSet responseDataTemp = new DataSet();
        try {
            // 1. FU lookup
            ...
            //DMS 홈화면 조회 용도
            //1. 공지사항리스트조회 (팝업제외)
            responseDataTemp = fSCAnncBrdMgmt.fInqAnncBrdLst(requestData, onlineCtx);
            responseData.putRecordSet("RS_ANNCE_LIST", responseDataTemp.getRecordSet("RS_ANNCE_LIST"));
            //2. 공지사항 팝업리스트 조회
            //팝업여부수정
            requestData.putField("POPUP_YN", "Y");
            responseDataTemp = fSCAnncBrdMgmt.fInqAnncBrdLst(requestData, onlineCtx);  
            responseData.putRecordSet("RS_ANNCE_POPUP_LIST", responseDataTemp.getRecordSet("RS_ANNCE_LIST"));                
            // 3. 결과값 리턴
            responseData.setOkResultMessage("DMS00001", null); //정상 조회되었습니다.
        } catch ( BizRuntimeException e ) {
            throw e;
