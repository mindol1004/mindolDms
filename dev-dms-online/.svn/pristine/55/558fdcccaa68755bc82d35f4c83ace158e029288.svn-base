...
public class PNRStlMnt extends fwk.base.ProcessUnit {
	 ...
	 * @return 처리결과 DataSet 객체
	 */
	public IDataSet pDelCalChrStlMgmt(IDataSet requestData, IOnlineContext onlineCtx) {
	    IDataSet responseData = new DataSet();
	
	    try {
            // 1. FM 호출
            responseData = callSharedBizComponentByDirect("nr.NRSXMBase", "fDelCalChrStlMgmt", requestData, onlineCtx);
        } catch ( BizRuntimeException e ) {
            throw e;
        } catch ( Exception e ) {
            throw new BizRuntimeException("DMS00009", e); // 시스템 오류가 발생하였습니다.
