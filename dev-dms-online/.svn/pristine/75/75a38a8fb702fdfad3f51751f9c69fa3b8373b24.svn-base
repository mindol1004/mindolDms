...
public class PNRCnslMngt extends fwk.base.ProcessUnit {
	...
	public IDataSet pInqScrbPrstLst(IDataSet requestData, IOnlineContext onlineCtx) {
        ...
        try {
            ...
            // 1. FM 호출 ("RS_SCRB_PG")
            responseData = callSharedBizComponentByDirect("nr.NRSCSBase", "fInqScrbPrstLst", reqDs, onlineCtx);
            // 2. 결과값 리턴
	 ...
	 *		- field : DEAL_TEL_NO_ENPT [거래처전번암호화]
	 *		- field : DEAL_EML_ADDR_ENPT [거래처이메일암호화]
	 *		- field : PRPB_NM_ENPT [담당자명암호화]
	 *		- field : PRPB_MBL_PHON_ENPT [담당자핸드폰암호화]
	 *		- field : NR_CNTRTR_NM_ENPT [계약자명암호화]
	 *		- field : NR_CNTRTR_BIRTH_ENPT [계약자생년월일암호화]
	 *		- field : NR_CNTRTR_INV_ADDR_DTL_ENPT [청구지주소상세암호화]
	 *		- field : LEGAL_REPVE_NM_ENPT [법정대리인명암호화]
	 *		- field : LEGAL_REPVE_BIRTH_ENPT [법정대리인생년월일암호화]
	 *		- field : OUT_PRC [출고가]
	 *	- record : RS_SUB
	 ...
	 *		- field : LS_CHG_DTM [최종변경일시]
	 *	- record : RS_POLY
	 *		- field : PEN_POLY_CL [구분]
	 *		- field : RP_OCCR_DT [발생일]
	 *		- field : DMS_PEN_AMT [DMS금액]
	 *		- field : UKEY_PEN_AMT [UKEY금액]
	 *		- field : RP_PAY_AMT [UKEY수납금액]
	 *		- field : RP_UNRTN_AMT [UKEY미납금액]
	 * </pre>
	 */
	public IDataSet pInqScrbPrstDetl(IDataSet requestData, IOnlineContext onlineCtx) {
        ...
        IDataSet reqDs = (IDataSet) requestData.clone();
        try {
            if ( StringUtils.isNotEmpty(reqDs.getField("CNTRT_NO")) || 
                 StringUtils.isNotEmpty(reqDs.getField("SVC_NO")) || 
                 StringUtils.isNotEmpty(reqDs.getField("SVC_MGMT_NO")) ) {
                if ( StringUtils.isNotEmpty(reqDs.getField("SVC_NO")) ) {
                    reqDs.putField("SVC_NO_ENPT", HpcUtils.encodeByAES(reqDs.getField("SVC_NO")));
                }
                // 1. FM 호출 ("RS_MAIN")
                responseData = callSharedBizComponentByDirect("nr.NRSCSBase", "fInqScrbPrstDetl", reqDs, onlineCtx);
                if ( StringUtils.isEmpty(reqDs.getField("CNTRT_NO")) ) {
                    IRecordSet rs = responseData.getRecordSet("RS_MAIN");
                    if (rs.getRecordCount() > 0) reqDs.putField("CNTRT_NO", rs.get(0, "CNTRT_NO"));
                }
                //(RS_SUB)
                //responseData.putRecordSet(callSharedBizComponentByDirect("nr.NRSCSBase", "fInqApplfSubmitDtmLst", reqDs, onlineCtx).getRecordSet("RS_SUB"));
                //(RS_SUB1)
                responseData.putRecordSet(callSharedBizComponentByDirect("nr.NRSCSBase", "fInqCnslMemoLst", reqDs, onlineCtx).getRecordSet("RS_SUB1"));
            }
            // 2. 결과값 리턴
            responseData.setOkResultMessage("DMS00001", null); //정상 조회되었습니다.
        } catch ( BizRuntimeException e ) {
    ...
    public IDataSet pInqRentalCnslLst(IDataSet requestData, IOnlineContext onlineCtx) {
        ...
        try {
            if ( StringUtils.isNotEmpty(reqDs.getField("SVC_NO")) ) {
                log.debug("SVC_NO["+reqDs.getField("SVC_NO")+"]");
                reqDs.putField("SVC_NO_ENPT", HpcUtils.encodeByAES(reqDs.getField("SVC_NO")));
            }
            // 1. FM 호출 ("RS_MAIN")
            if ( StringUtils.isNotEmpty(reqDs.getField("CNSL_CL")) && "R".equals(reqDs.getField("CNSL_CL")) ) {
                responseData = callSharedBizComponentByDirect("nr.NRSCSBase", "fInqRentalCnslLst", reqDs, onlineCtx);
            } else {
                responseData = callSharedBizComponentByDirect("nr.NRSCSBase", "fInqEqpInsuCnslLst", reqDs, onlineCtx);
            }
            // 2. 결과값 리턴
            responseData.setOkResultMessage("DMS00001", null); //정상 조회되었습니다.
        } catch ( BizRuntimeException e ) {
    ...
    public IDataSet pSaveRentalCnsl(IDataSet requestData, IOnlineContext onlineCtx) {
        ...
        CommonArea ca = getCommonArea(onlineCtx);
        IDataSet reqDs = (IDataSet) requestData.clone();
        Map recordMap = requestData.getRecordSet("RS_MAIN").getRecordMap(0);
        
        try {
            reqDs.putFieldMap(requestData.getRecordSet("RS_MAIN").getRecordMap(0));
            reqDs.putField("USERNO", ca.getUserNo());
            reqDs.putField("SVC_NO_ENPT", HpcUtils.encodeByAES((String)recordMap.get("SVC_NO")));
            reqDs.putField("SVC_NO", HpcUtils.maskingTelNo((String)recordMap.get("SVC_NO")));
            reqDs.putField("OK_RWD_ACCO_ENPT", HpcUtils.encodeByAES((String)recordMap.get("OK_RWD_ACCO")));
            reqDs.putField("OK_RWD_ACCO", HpcUtils.maskingAccountNo((String)recordMap.get("OK_RWD_ACCO")));
            // 1. FM 호출 ("RS_MAIN")
            if ( StringUtils.isNotEmpty(reqDs.getField("CNSL_NO")) ) {
                if ( StringUtils.isNotEmpty((String)recordMap.get("CNSL_CL")) && "R".equals((String)recordMap.get("CNSL_CL")) ) {
                    responseData = callSharedBizComponentByDirect("nr.NRSCSBase", "fUpdRentalCnsl", reqDs, onlineCtx);
                } else {
                    responseData = callSharedBizComponentByDirect("nr.NRSCSBase", "fUpdEqpInsuCnsl", reqDs, onlineCtx);
                }
            } else {
                responseData = callSharedBizComponentByDirect("nr.NRSCSBase", "fInqRentalCnslCntrtNo", reqDs, onlineCtx);
                if(responseData.getField("CNTRT_NO").length() == 0) {
                    //throw new BizRuntimeException("DMS00116"); //입력하신 서비스번호에 해당하는 렌탈계약이 없습니다.
                    if ( StringUtils.isNotEmpty((String)recordMap.get("CNSL_CL")) && "R".equals((String)recordMap.get("CNSL_CL")) ) {
                        responseData = callSharedBizComponentByDirect("nr.NRSCSBase", "fRegRentalCnslNEW", reqDs, onlineCtx);
                    } else {
                        responseData = callSharedBizComponentByDirect("nr.NRSCSBase", "fRegEqpInsuCnslNEW", reqDs, onlineCtx);
                    }
                } else {
                    if ( StringUtils.isNotEmpty((String)recordMap.get("CNSL_CL")) && "R".equals((String)recordMap.get("CNSL_CL")) ) {
                        responseData = callSharedBizComponentByDirect("nr.NRSCSBase", "fRegRentalCnsl", reqDs, onlineCtx);
                    } else {
                        responseData = callSharedBizComponentByDirect("nr.NRSCSBase", "fRegEqpInsuCnsl", reqDs, onlineCtx);
                    }
                }
            }
