 ...
 */
public class PEPSellGradeMgmt extends fwk.base.ProcessUnit {
	 ...
	 *		- field : EQP_ST_RMK [단말기 감정사유]
	 * </pre>
	 */
	public IDataSet pInqSellGradeList(IDataSet requestData, IOnlineContext onlineCtx) {
	    IDataSet responseData = new DataSet();
        
        try {
            // 1. FM 호출
            responseData = callSharedBizComponentByDirect("ep.EPSCSBase", "fInqSellGradeList", requestData, onlineCtx);
	 ...
	 * @return 처리결과 DataSet 객체
	 */
	public IDataSet pSaveSellGrade(IDataSet requestData, IOnlineContext onlineCtx) {
	    IDataSet responseData = new DataSet();
        CommonArea ca = getCommonArea(onlineCtx);// 처리 결과값을 responseData에 넣어서 리턴하십시요 
        try {
            requestData.putField("USER_NO", ca.getUserNo());
            // 1. FM 호출
            responseData = callSharedBizComponentByDirect("ep.EPSCSBase", "fRegSellGrade", requestData, onlineCtx);
        } catch ( BizRuntimeException e ) {
            throw e;
        } catch ( Exception e ) {
            throw new BizRuntimeException("DMS00009", e); // 시스템 오류가 발생하였습니다.
	 ...
	 * </pre>
	 */
	public IDataSet pInqSellGrade(IDataSet requestData, IOnlineContext onlineCtx) {
	    IDataSet responseData = new DataSet();
        
        try {
            // 1. FM 호출
            responseData = callSharedBizComponentByDirect("ep.EPSCSBase", "fInqSellGrade", requestData, onlineCtx);
        } catch ( BizRuntimeException e ) {
            throw e;
        } catch ( Exception e ) {
            throw new BizRuntimeException("DMS00009", e); // 시스템 오류가 발생하였습니다.
