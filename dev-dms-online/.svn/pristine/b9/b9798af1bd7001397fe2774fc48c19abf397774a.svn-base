...
//				logger.debug(">>>>>>>>>>>>>>>>>>   mask is  :"+ mMgmt.get("DPSTR").toString().indexOf("*"));
				...
				if (sSeq == null || "".equals(sSeq)) {
					...
					mRmks.put("CUST_IDEA",StringUtils.nvlObject(mRmks.get("CUST_IDEA"), ""));
					mRmks.put("CUST_IDEA_DTL",StringUtils.nvlObject(mRmks.get("CUST_IDEA_DTL"), ""));
