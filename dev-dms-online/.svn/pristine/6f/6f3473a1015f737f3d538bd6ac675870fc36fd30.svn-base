...
public class PEPReportMgmt extends fwk.base.ProcessUnit {
	 ...
	 *		- field : SKN_CL [SKN 구분]
	 *		- field : ROWNO [번호]
	 *		- field : INVE_PCOST_SLIP_NO [재고원가전표번호]
	 *		- field : PAY_SLIP_DT [지급전표기산일자]
	 ...
	 *		- field : DATA_GB [데이터구분]
	 *		- field : DATA_SECTION [기준데이터구분]
	 *		- field : DATA_GB_SORT [데이터구분정렬]
	 *		- field : DATA_SECTION_SORT [기준데이터구분정렬]
	 *		- field : CNSL_CNT [보고서데이터]
	 * </pre>
	 */
	public IDataSet pInqNewCmpChgSaleListPivot(IDataSet requestData, IOnlineContext onlineCtx) {
        IDataSet responseData = new DataSet();
        
        try {
            // 1. FM 호출
            responseData = callSharedBizComponentByDirect("ep.EPSRPBase", "fInqNewCmpChgSaleListPivot", requestData, onlineCtx);
        } catch ( BizRuntimeException e ) {
            throw e;
        } catch ( Exception e ) {
            throw new BizRuntimeException("DMS00009", e); // 시스템 오류가 발생하였습니다.
	 ...
	 * </pre>
	 */
	public IDataSet pInqTEchoSaleListPivot(IDataSet requestData, IOnlineContext onlineCtx) {
	    IDataSet responseData = new DataSet();
        
        try {
            // 1. FM 호출
            responseData = callSharedBizComponentByDirect("ep.EPSRPBase", "fInqTEchoSaleListPivot", requestData, onlineCtx);
        } catch ( BizRuntimeException e ) {
            throw e;
        } catch ( Exception e ) {
            throw new BizRuntimeException("DMS00009", e); // 시스템 오류가 발생하였습니다.
	 ...
	 * </pre>
	 */
	public IDataSet pInqClubTSaleListPivot(IDataSet requestData, IOnlineContext onlineCtx) {
	    IDataSet responseData = new DataSet();
        
        try {
            // 1. FM 호출
            responseData = callSharedBizComponentByDirect("ep.EPSRPBase", "fInqClubTSaleListPivot", requestData, onlineCtx);
        } catch ( BizRuntimeException e ) {
            throw e;
        } catch ( Exception e ) {
            throw new BizRuntimeException("DMS00009", e); // 시스템 오류가 발생하였습니다.
