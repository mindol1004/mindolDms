...
public class FEPEqpMdlPriceMgmt extends fwk.base.FunctionUnit {
	 ...
	 * </pre>
	 */
	public IDataSet fInqCmptPriceLst(IDataSet requestData, IOnlineContext onlineCtx) {
		 IDataSet responseData = new DataSet();
	        ...
	        try {
	            // 1. DU lookup
	        	 DEPEqpMdlPriceMgmt dEPEqpMdlPriceMgmt = (DEPEqpMdlPriceMgmt) lookupDataUnit(DEPEqpMdlPriceMgmt.class);
	            // 2. 업무 DM호출
	            responseData = dEPEqpMdlPriceMgmt.dSfInqCmptPriceLst(requestData, onlineCtx);
	        } catch ( BizRuntimeException e ) {
	            throw e;
	        }
