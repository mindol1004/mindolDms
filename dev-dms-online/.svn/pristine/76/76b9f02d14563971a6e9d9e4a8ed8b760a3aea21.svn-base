...
public class PNREqpJdgDTlInfoRgst extends fwk.base.ProcessUnit {
	 ...
	 *		- field : ASSET_NO [자산번호]
	 *		- field : USE_YN [사용여부]
	 *		- field : EQP_CMP_REVIS_AMT [단말기배상보정금액]
	 *		- field : EQP_JDG_RMK [비고]
	 *	- record : RS_EQP_EVAL_INFO_REG_YN
	 *		- field : SVC_MGMT_NO_CNT [필드1]
	 * </pre>
	 */
	public IDataSet pInqReqpJdgDtlRgstLst(IDataSet requestData, IOnlineContext onlineCtx) {
	    ...
	    try {            	
            	if(responseData.getRecordSet("RS_REQP_JDG_LIST2").getRecordCount() > 0
            		&& !"".equals(responseData.getRecordSet("RS_REQP_JDG_LIST2").getRecord(0).get("ATCH_FILE_NO")) ){
            		requestData.putField("UPLD_FILE_NO"  , responseData.getRecordSet("RS_REQP_JDG_LIST2").getRecord(0).get("ATCH_FILE_NO"));
	 ...
	 *	- field : EQP_IN_DT [감정일자]
	 *	- field : EQP_CMP_REVIS_AMT [단말기배상보정금액]
	 *	- field : EQP_JDG_RMK [비고]
	 *	- field : DCP_INSP_DTL_ITM_CD [필드1]
	 * </pre>
	 * @param onlineCtx   요청 컨텍스트 정보
	 * @return 처리결과 DataSet 객체
	 */
