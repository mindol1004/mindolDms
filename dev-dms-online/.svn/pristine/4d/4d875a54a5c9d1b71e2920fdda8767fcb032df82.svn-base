...
public class FEPEqpAmtRmtXclMgmt extends fwk.base.FunctionUnit {
        ...
        try {
            ...
            if (excelRs != null) {
                ...
                for (int i = 0; i < excelRs.getRecordCount(); i++) {
                    ...
                    if(excelRd.get("XCL_STRD_STA_DT").trim().length() != 8 || excelRd.get("XCL_STRD_END_DT").trim().length() != 8){
                        excelRd.set("ERR_DESC", "정산기준일은 (YYYYMMDD)으로 입력 하셔야 합니다.");
                        continue;
                    }
                    
                    if(!StringUtils.isNumeric(excelRd.get("XCL_STRD_STA_DT").trim()) || !StringUtils.isNumeric(excelRd.get("XCL_STRD_END_DT").trim())){
                        excelRd.set("ERR_DESC", "정산기준일은 숫자형식으로 입력 하셔야 합니다.");
                        continue;
                    }
                    
                    if(i==0){
                        XCL_STRD_STA_DT = excelRd.get("XCL_STRD_STA_DT").trim();
                        XCL_STRD_END_DT = excelRd.get("XCL_STRD_END_DT").trim();
                    }
                    
                    if(!StringUtils.equals(XCL_STRD_STA_DT, excelRd.get("XCL_STRD_STA_DT").trim()) || !StringUtils.equals(XCL_STRD_END_DT, excelRd.get("XCL_STRD_END_DT").trim())){
                        excelRd.set("ERR_DESC", "정산기준일이 일치하지 않습니다.");
                        continue;
                    }
                    
                    if(excelRd.get("INVE_TYP_NM") == null){
                        excelRd.set("ERR_DESC", "구분값이 없습니다.");
                        continue;
                    }
                    
                    if(!StringUtils.equals(excelRd.get("INVE_TYP_NM").trim() ,"현장") && !StringUtils.equals(excelRd.get("INVE_TYP_NM").trim() ,"비현장")){
                    ...
                    reqDs.putField("STRD_FR", XCL_STRD_STA_DT);
                    reqDs.putField("STRD_TO", XCL_STRD_END_DT);
                    reqDs.putField("CNSL_MGMT_NO", excelRd.get("CNSL_MGMT_NO").trim());
                    reqDs.putField("CNCL_SLIP_YN", "Y");
                    ...
                    IDataSet slipCnt = dEPEqpAmtRmtXclMgmt.dSEqpAmtRmtXclChk(reqDs, onlineCtx);
                    if( slipCnt.getIntField("CNT") > 0){
                        //excelRd.set("ERR_DESC", "전표처리된 데이터 입니다.");
                        excelRd.set("ERR_DESC", "해당 정산기준 기간은 반제전표가 등록된 기간 입니다.");
                        continue;
                    }
                    
                    IDataSet dsCnt = dEPEqpAmtRmtXclMgmt.dSEqpAmtRmtXclExcelChk(reqDs, onlineCtx);
                    if( dsCnt.getIntField("CNT") > 0){
                        excelRd.set("ERR_DESC", "해당기준일자에 반제전표로 등록된 데이터 입니다.");
                        continue;
                    }
                    
                    if(StringUtils.equals(excelRd.get("INVE_TYP_NM").trim() ,"현장")){
                        INVE_TYP_CD = "20";
                    }else if(StringUtils.equals(excelRd.get("INVE_TYP_NM").trim() ,"비현장")){
                        INVE_TYP_CD = "10";
                    }
                    
                    if(StringUtils.equals(xclDs.getField("CNSL_DEALCO_CD") ,"11050")){
                        INVE_TYP_CD = "30";
                    }
                    
                    excelRd.set("XCL_STRD_STA_DT", excelRd.get("XCL_STRD_STA_DT").trim()); //정산기준시작일
                    excelRd.set("XCL_STRD_END_DT", excelRd.get("XCL_STRD_END_DT").trim()); //정산기준종료일
                    excelRd.set("INVE_TYP_CD", INVE_TYP_CD); //재고유형
                    excelRd.set("INVE_TYP_NM", excelRd.get("INVE_TYP_NM").trim()); //재고유형명
                    ...
                    excelRd.set("RMT_DT", xclDs.getField("RMT_DT")); //송금일자
	...
	public IDataSet fSaveEqpAmtRmtXclExcel(IDataSet requestData, IOnlineContext onlineCtx) {
        ...
        try {
            ...
            for (int i = 0; i < excelRs.getRecordCount(); i++) {
                excelRd = excelRs.getRecord(i);
                if(StringUtils.equals(excelRd.get("INVE_TYP_CD") ,"10")){
                    ACCO_RMT_XCL_CNT++;
            ...
            if(result){
                ...
                reqDs.putField("STRD_FR", XCL_STRD_STA_DT);
                reqDs.putField("STRD_TO", XCL_STRD_END_DT);
                reqDs.putField("INVE_TYP_CD", INVE_TYP_CD);
                IRecordSet delRs = dEPEqpAmtRmtXclMgmt.dSEqpAmtRmtXclSeq(reqDs, onlineCtx).getRecordSet("RS_EQP_ACCO_RMT_SEQ");
                IDataSet delDs = null;
                if (delRs.getRecordCount() > 0) {
                    for (int i = 0; i < delRs.getRecordCount(); i++) {
                        ...
                        dEPEqpAmtRmtXclMgmt.dUEqpAmtRmtXclDel(delDs, onlineCtx);
                        dEPEqpAmtRmtXclMgmt.dUEqpAmtRmtXclDtlDel(delDs, onlineCtx);
                    }
                }
            ...
            for (int i = 0; i < excelRs.getRecordCount(); i++) {
                excelRd = excelRs.getRecord(i);
                if(StringUtils.equals(excelRd.get("INVE_TYP_CD") ,"20")){
                    ACCO_RMT_XCL_CNT++;
                    ACCO_RMT_AMT += Integer.parseInt(excelRd.get("XCL_AMT"));
                    INVE_TYP_CD = excelRd.get("INVE_TYP_CD");
                    XCL_STRD_STA_DT = excelRd.get("XCL_STRD_STA_DT");
            ...
            for (int i = 0; i < excelRs.getRecordCount(); i++) {
                excelRd = excelRs.getRecord(i);
                if(StringUtils.equals(excelRd.get("INVE_TYP_CD") ,"30")){
                    ACCO_RMT_XCL_CNT++;
                    ACCO_RMT_AMT += Integer.parseInt(excelRd.get("XCL_AMT"));
                    INVE_TYP_CD = excelRd.get("INVE_TYP_CD");
                    XCL_STRD_STA_DT = excelRd.get("XCL_STRD_STA_DT");
