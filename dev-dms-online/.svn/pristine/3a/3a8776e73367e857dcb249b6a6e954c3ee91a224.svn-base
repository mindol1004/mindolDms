...
public class FNRErpIF extends fwk.base.FunctionUnit {
	 ...
	 * @return 처리결과 DataSet 객체
	 */
	public IDataSet fInqErpDealCoSt(IDataSet requestData, IOnlineContext onlineCtx) {
		IDataSet responseData = new DataSet();
	    String jobExecutionId = ""; // 실행 JOB_ID	
	    try {
	    	IRecordSet rs = requestData.getRecordSet("RS_SLIP_LIST");
	    	
	    	int bizIdx = rs.getHeaderIndex("BIZ_REG_NO");
	    	int blicensIdx = rs.getHeaderIndex("DEALCO_BLICENS_NO");
	    	...
	    	if( bizIdx < 0 ){
	    		...
	    		rs.addHeader( new RecordHeader("BIZ_REG_NO") );
	    		
	    		Iterator<IRecord> itr = rs.getRecords();
	    		IRecord record = null;
	    		while( itr.hasNext()){
	    			record = itr.next();
	    			record.put("BIZ_REG_NO", record.get("DEALCO_BLICENS_NO"));
	    		}
