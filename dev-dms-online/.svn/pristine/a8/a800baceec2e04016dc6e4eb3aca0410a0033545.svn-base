...
public class FNRFpaCmmsXclMgmt extends fwk.base.FunctionUnit {
	...
	public IDataSet fFpaCmmsXclDeprOnlineRecall(IDataSet requestData, IOnlineContext onlineCtx) {
        ...
        CommonArea ca = getCommonArea(onlineCtx);
        try {
            IDataSet reqDs = this.fFpaCmmsXclLst(requestData, onlineCtx);
            IRecordSet fpaCmmRs = reqDs.getRecordSet("RS_FPA_CMMS_XCL_LIST");
            IRecordSet sumRs = reqDs.getRecordSet("RS_SUM_LIST");
            
            if( fpaCmmRs == null ){
                return responseData;
            }
            requestData.putRecordSet(fpaCmmRs);
            requestData.putRecordSet(sumRs);
        
            for(int i=0; i<fpaCmmRs.getRecordCount(); i++){
                IRecord ir = fpaCmmRs.getRecord(i);
            ...
            requestData.putField("CNT", totalSum.get("TOTAL_CNT"));     //건수
            requestData.putField("AMT", totalSum.get("FPA_CMMS_AMT"));  //금액
            requestData.putField("BIZ_REG_NO", totalSum.get("DEALCO_BLICENS_NO"));//사업자번호
            //requestData.putField("BIZ_REG_NO", "1000000008");         //사업자번호
            requestData.putField("YYYYMM", totalSum.get("YYYYMM"));     //전표년월
            requestData.putField("SLIP_TYPE", "NR_FP");                 //전표유형
...
//            requestData.getRecordSet("RS_FPA_CMMS_XCL_LIST").setId("RS_SLIP_DELETE");//SLIP_NO,YYYY들어있음
//            IRecordSet rs  = requestData.getRecordSet("RS_FPA_CMMS_XCL_LIST");
//            requestData.putRecordSet("RS_SLIP_DELETE", (IRecordSet) rs.clone());
//            requestData.removeRecordSet("RS_FPA_CMMS_XCL_LIST");

            IRecord totalSum = requestData.getRecordSet("RS_SUM_LIST").getRecord(0);

            requestData.putField("YYYY", totalSum.get("YYYY"));         //전표년도
            requestData.putField("YYYYMM", totalSum.get("YYYYMM"));     //전표년월
            requestData.putField("SLIP_NO", totalSum.get("SLIP_NO"));   //전표번호
            requestData.putField("SLIP_TYPE", "NR_FP");                 //전표유형
            requestData.putField("USER_NO", ca.getUserNo());
