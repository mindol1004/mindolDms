...
public class PNRInsuInvMgmt extends fwk.base.ProcessUnit {
	 ...
	 *		- field : INVE_MATCHED [청구금액일치여부]
	 *		- field : CASE_WHEN [구분값]
	 *		- field : CBACK_OBJ_YN [필드1]
	 *		- field : CBACK_ST_CD [필드2]
	 *		- field : CBACK_ST_DT [필드3]
	 * </pre>
	 * @param onlineCtx   요청 컨텍스트 정보
	 * @return 처리결과 DataSet 객체
	 */
	 ...
	 * @param requestData 요청정보 DataSet 객체
	 * <pre>
	 *	- field : PROC_DT [동기화처리일자]
	 * </pre>
	 * @param onlineCtx   요청 컨텍스트 정보
	 * @return 처리결과 DataSet 객체
	 */
	public IDataSet pInqInsuInvStSync(IDataSet requestData, IOnlineContext onlineCtx) {
        IDataSet responseData = new DataSet();
    
        try {
            // 1. FM 호출
            responseData = callSharedBizComponentByDirect("nr.NRSISBase", "fInqInsuInvStSync", requestData, onlineCtx);
        } catch ( BizRuntimeException e ) {
            throw e;
        } catch ( Exception e ) {
            throw new BizRuntimeException("DMS00009", e); // 시스템 오류가 발생하였습니다.
