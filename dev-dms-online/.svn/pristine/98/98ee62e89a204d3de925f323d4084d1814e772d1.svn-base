...
public class DNRPenPolyMgmt extends fwk.base.DataUnit {
	 ...
	 *
	 * @author 김혁범 (kezmain1)
	 * @since 2015-07-16 16:53:58
	 *
	 * @param requestData 요청정보 DataSet 객체
	 * @param onlineCtx   요청 컨텍스트 정보
	 * @return 처리결과 DataSet 객체
	 */
	public IDataSet dUPenPolyDate(IDataSet requestData, IOnlineContext onlineCtx) {
	    IDataSet responseData = new DataSet();
	
	    // 1.쿼리문 호출
	    dbUpdate("UPenPolyDate", requestData, onlineCtx);
	
	    return responseData;
	}

	/**
	 *위약금정책정보  신규등록시 유효기간시작일 체크
	 *
	 * @author 김혁범 (kezmain1)
	 * @since 2015-07-20 13:38:34
	 *
	 * @param requestData 요청정보 DataSet 객체
	 * @param onlineCtx   요청 컨텍스트 정보
	 * @return 처리결과 DataSet 객체
	 */
	public IDataSet dSChkPolyFrDt(IDataSet requestData, IOnlineContext onlineCtx) {
	    IDataSet responseData = new DataSet();
	    
	    // 1.쿼리문 호출	    
  		IRecord record = dbSelectSingle("SChkPolyFrDt", requestData, onlineCtx);
  		// 2.결과값 셋팅	    
  		responseData.putFieldMap(record); 
	...
	/**
	 *위약금정책정보 수정시 유효기간시작일 체크
	 *
	 * @author 김혁범 (kezmain1)
	 * @since 2015-07-20 14:35:12
	 *
	 * @param requestData 요청정보 DataSet 객체
	 * @param onlineCtx   요청 컨텍스트 정보
	 * @return 처리결과 DataSet 객체
	 */
	public IDataSet dSChkPolyToDt(IDataSet requestData, IOnlineContext onlineCtx) {
	    IDataSet responseData = new DataSet();
	
	    // 1.쿼리문 호출	    
  		IRecord record = dbSelectSingle("SChkPolyToDt", requestData, onlineCtx);
  		// 2.결과값 셋팅	    
  		responseData.putFieldMap(record); 
