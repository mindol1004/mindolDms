...
public class PNRCmpStdMgmt extends fwk.base.ProcessUnit {
	...
	public IDataSet pSaveCmpStd(IDataSet requestData, IOnlineContext onlineCtx) {
        ...
        try {
            ...
            //신규일 경우 같은 기등록된 모델이 있는지 체크
            if ( "I".equals(ca.getTrnPtrnDvcd()) ){
                IDataSet validDs = callSharedBizComponentByDirect("nr.NRSPLBase", "fInqDupMdl", requestData, onlineCtx);
                if( validDs.getIntField("CNT") > 0 ){
