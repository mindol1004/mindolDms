...
public class DPRRentalServCntrtMgmt extends fwk.base.DataUnit {
    ...
    /**
	 * <pre>단말기렌탈계약기간(포괄)수정</pre>
	 *
	 * @author 양재석 (jsyang)
	 * @since 2015-08-25 12:59:48
	 *
	 * @param requestData 요청정보 DataSet 객체
	 * @param onlineCtx   요청 컨텍스트 정보
	 * @return 처리결과 DataSet 객체
	 */
	public IDataSet dUEqpRentalCntrtDtCmphsUpd(IDataSet requestData, IOnlineContext onlineCtx) {
        IDataSet responseData = new DataSet();
    
        try {       
            dbInsert("UEqpRentalCntrtDtCmphsUpd", requestData, onlineCtx); 
        }
        catch ( BizRuntimeException e ) {
            throw e;
        } 
	 ...
	 * @return 처리결과 DataSet 객체
	 * <pre>
	 *	- field : CNTRT_CNT [매칭계약건수]
	 * </pre>
	 */
	public IDataSet dSInqCmphsCntrtJoinCnt(IDataSet requestData, IOnlineContext onlineCtx) {
        IDataSet responseData = new DataSet();
        
        // 1.쿼리문 호출     
        IRecord record = dbSelectSingle("SInqCmphsCntrtJoinCnt", requestData, onlineCtx);
        // 2.결과값 셋팅     
        responseData.putFieldMap(record);
	 ...
	 * </pre>
	 */
	public IDataSet dSInqEqpCntrtJoinCnt(IDataSet requestData, IOnlineContext onlineCtx) {
        IDataSet responseData = new DataSet();
    
        // 1.쿼리문 호출     
        IRecord record = dbSelectSingle("SInqEqpCntrtJoinCnt", requestData, onlineCtx);
        // 2.결과값 셋팅     
        responseData.putFieldMap(record);
