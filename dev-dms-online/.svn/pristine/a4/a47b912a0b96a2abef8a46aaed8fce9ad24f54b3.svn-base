		...
		try {
			...
			//if ( StringUtils.equals(ca.getFrstTrnmChnlCd(), "XIUI")) {
				...
				if ( !StringUtils.equals(onlineCtx.getTransaction().getTxId(), "PSCLginOp_pLgin")
                    && !StringUtils.equals(onlineCtx.getTransaction().getTxId(), "PSCLginOp_pLginWeb")				
					...
					if ( StringUtils.equals(ca.getFrstTrnmChnlCd(), "XIUI")) {
					    ...
					    reqDsSeon.putField("SESS_NO", onlineCtx.getUserInfo().get("SESS_NO"));
                        reqDsSeon.putField("USER_NO", onlineCtx.getUserInfo().get("USER_NO"));
                        reqDsSeon.putField("IP_ADDR", onlineCtx.getUserInfo().getIp());
					}
					...
					ca.setUserNm(responseData.getField("USER_NM"));         // 사용자이름
					...
					ca.setDealCd(responseData.getField("DEALCO_CD"));         // 대리점코드
