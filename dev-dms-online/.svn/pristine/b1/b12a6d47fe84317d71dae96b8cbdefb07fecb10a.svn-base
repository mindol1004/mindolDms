...
public class DEPFPAProxySaleMgmt extends fwk.base.DataUnit {
    ...
    /**
	 * <pre>FPA판매대행상품판매취소총건수</pre>
	 *
	 * @author 이민재 (mindol1004)
	 * @since 2016-02-25 10:06:39
	 *
	 * @param requestData 요청정보 DataSet 객체
	 * @param onlineCtx   요청 컨텍스트 정보
	 * @return 처리결과 DataSet 객체
	 */
	public IDataSet dSFpaProxySaleCnclListTotCnt(IDataSet requestData, IOnlineContext onlineCtx) {
        IDataSet responseData = new DataSet();
    
        // 1.쿼리문 호출     
        IRecord record = dbSelectSingle("SFpaProxySaleCnclListTotCnt", requestData, onlineCtx);
        // 2.결과값 셋팅     
        responseData.putFieldMap(record); 
	 ...
	 *
	 * @author 이민재 (mindol1004)
	 * @since 2016-02-25 10:07:12
	 *
	 * @param requestData 요청정보 DataSet 객체
	 * @param onlineCtx   요청 컨텍스트 정보
	 * @return 처리결과 DataSet 객체
	 */
	public IDataSet dSFpaProxySaleCnclListPaging(IDataSet requestData, IOnlineContext onlineCtx) {
        IDataSet responseData = new DataSet();
    
        // 1.쿼리문 호출
        IRecordSet rsReturn = dbSelect("SFpaProxySaleCnclListPaging", requestData, onlineCtx);
        // 2.결과값 셋팅
        responseData.putRecordSet("RS_FPA_PROXY_CNCL_LIST", rsReturn); 
    
        return responseData;
    }

    /**
	 * <pre>FPA판매대행상품판매취소일반판매저장</pre>
	 *
	 * @author 이민재 (mindol1004)
	 * @since 2016-02-25 15:15:25
	 *
	 * @param requestData 요청정보 DataSet 객체
	 * @param onlineCtx   요청 컨텍스트 정보
	 * @return 처리결과 DataSet 객체
	 */
	public IDataSet dIFpaProxyGnrlSaleCncl(IDataSet requestData, IOnlineContext onlineCtx) {
        IDataSet responseData = new DataSet();
    
        // 1.쿼리문 호출
        dbInsert("IFpaProxyGnrlSaleCncl", requestData, onlineCtx);
    
        return responseData;
    }

    /**
	 * <pre>FPA판매대행상품판매취소단말기판매저장</pre>
	 *
	 * @author 이민재 (mindol1004)
	 * @since 2016-02-25 15:15:59
	 *
	 * @param requestData 요청정보 DataSet 객체
	 * @param onlineCtx   요청 컨텍스트 정보
	 * @return 처리결과 DataSet 객체
	 */
	public IDataSet dIFpaProxySaleEquipmentCncl(IDataSet requestData, IOnlineContext onlineCtx) {
        IDataSet responseData = new DataSet();
    
        // 1.쿼리문 호출
        dbInsert("IFpaProxySaleEquipmentCncl", requestData, onlineCtx);
    
        return responseData;
    }

    /**
	 * <pre>FPA판매대행상품판매취소단말기판매수정</pre>
	 *
	 * @author 이민재 (mindol1004)
	 * @since 2016-02-25 15:16:38
	 *
	 * @param requestData 요청정보 DataSet 객체
	 * @param onlineCtx   요청 컨텍스트 정보
	 * @return 처리결과 DataSet 객체
	 */
	public IDataSet dUFpaProxySaleEquipmentCncl(IDataSet requestData, IOnlineContext onlineCtx) {
        IDataSet responseData = new DataSet();
    
        // 1.쿼리문 호출
        dbUpdate("UFpaProxySaleEquipmentCncl", requestData, onlineCtx);
    
        return responseData;
    }

    /**
	 * <pre>FPA판매대행상품판매건수조회</pre>
	 *
	 * @author 이민재 (mindol1004)
	 * @since 2016-02-25 15:17:17
	 *
	 * @param requestData 요청정보 DataSet 객체
	 * @param onlineCtx   요청 컨텍스트 정보
	 * @return 처리결과 DataSet 객체
	 */
	public IDataSet dSFpaProxyGnrlSaleCount(IDataSet requestData, IOnlineContext onlineCtx) {
        IDataSet responseData = new DataSet();
    
        // 1.쿼리문 호출     
        IRecord record = dbSelectSingle("SFpaProxyGnrlSaleCount", requestData, onlineCtx);
        // 2.결과값 셋팅     
        responseData.putFieldMap(record);
