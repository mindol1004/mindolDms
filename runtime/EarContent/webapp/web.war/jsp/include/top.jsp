...
<c:set var="downloadFileId" value="<%= Constants.UPLOADED_FILE_ID %>" />
<c:set var="downloadFileNm" value="<%= Constants.UPLOADED_FILE_NAME%>" />
...
<script>
	...
	function fn_openChgPwdPopup(){
		 window.open('/jsp/chgPwdPopup.jsp', 'pop','toolbar=no,directories=no,scrollbars=no,resizable=no,status=no,menubar=no,width=395,height=460');		 
	}
